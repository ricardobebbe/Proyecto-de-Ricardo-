...
LRESULT CALLBACK WndProc(HWND hWnd,UINT message,WPARAM wParam,LPARAM lParam) // OK
{
	switch(message)
	{
		case WM_COMMAND:
			switch(LOWORD(wParam))
			{
				case IDM_ABOUT:
					...
					break;
				case IDM_EXIT:
					if(MessageBox(0,"Estas  Seguro de terminar La secion de ConnectServer?","ATENCION QUERES CERRAR",MB_YESNO | MB_ICONQUESTION) == IDYES)
					{
						DestroyWindow(hWnd);
					}
					break;
		...
		case WM_DESTROY:
			PostQuitMessage(0);
			break;
			case WM_PAINT:     //RDDLV up24 banner personalizado
		{
