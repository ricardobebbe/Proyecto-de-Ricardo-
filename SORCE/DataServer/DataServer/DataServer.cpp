...
int APIENTRY WinMain(HINSTANCE hInstance,HINSTANCE hPrevInstance,LPSTR lpCmdLine,int nCmdShow) // OK
{
	...
	if(WSAStartup(MAKEWORD(2,2),&wsa) == 0)
	{
		...
		{
			LogAdd(LOG_RED,"Não foi possível se conectar ao banco de dados");
		}
		else
		{
	...
	{
		LogAdd(LOG_RED,"WSAStartup() falhou com erro: %d",WSAGetLastError());
	}
...
LRESULT CALLBACK WndProc(HWND hWnd,UINT message,WPARAM wParam,LPARAM lParam) // OK
{
	switch(message)
	{
		case WM_COMMAND:
			switch(LOWORD(wParam))
			{
				case IDM_ABOUT:
					...
					break;
				case IDM_EXIT:
					if(MessageBox(0,"Estas  Seguro de terminar La secion de DatatServer?","ATENCION QUERES CERRAR",MB_YESNO | MB_ICONQUESTION) == IDYES)
					{
						DestroyWindow(hWnd);
					}
					break;
		...
		case WM_TIMER:
			...
			break;
		case WM_CLOSE:
			if (MessageBox(0, "Estas  Seguro de terminar La secion de DatatServer?","ATENCION QUERES CERRAR", MB_OKCANCEL) == IDOK)
			{
				DestroyWindow(hWnd);
			}
			break;
