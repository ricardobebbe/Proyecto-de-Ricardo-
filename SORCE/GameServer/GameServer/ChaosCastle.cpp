...
void CChaosCastle::SetState_START(CHAOS_CASTLE_LEVEL* lpLevel) // OK
{
	...
	switch(lpLevel->Level)
	{
		case 0:
			...
			break;
		case 1:
			this->SetMonsterLevelTwo(lpLevel,164);
			this->SetMonsterLevelTwo(lpLevel,165);
			break;
		case 2:
			this->SetMonsterLevelThe(lpLevel,166);
			this->SetMonsterLevelThe(lpLevel,167);
			break;
		case 3:
			this->SetMonsterLevelFor(lpLevel,168);
			this->SetMonsterLevelFor(lpLevel,169);
			break;
		case 4:
			this->SetMonsterLevelFiv(lpLevel,170);
			this->SetMonsterLevelFiv(lpLevel,171);
			break;
		case 5:
			this->SetMonsterLevelSix(lpLevel,172);
			this->SetMonsterLevelSix(lpLevel,173);
			break;
		case 6:
			this->SetMonsterLevelSev(lpLevel,426);
			this->SetMonsterLevelSev(lpLevel,427);
			break;
	}
...
int CChaosCastle::GetUserAbleLevel(LPOBJ lpObj) // OK
{
	...
	if(lpObj->Class == CLASS_MG || lpObj->Class == CLASS_DL || lpObj->Class == CLASS_RF)
	{
		if(lpObj->ChangeUp < 2)
		{
			...
			else if(lpObj->Level >= 280 && lpObj->Level <= 500)
			{
				level = 5;
			}
		}
	...
	else
	{
		if(lpObj->ChangeUp < 2)
		{
			...
			else if(lpObj->Level >= 300 && lpObj->Level <= 500)
			{
				level = 5;
			}
		}
...
{
	for(int n=0;n < gMonsterSetBaseChaosCastle.m_count;n++)
	{
		...
		MONSTER_SET_BASE_INFO_CHAOSCASTLE1* lpInfo = &gMonsterSetBaseChaosCastle.m_MonsterSetBaseInfo[n];

		if(lpInfo->Type != 4 || lpInfo->MonsterClass != MonsterClass || lpInfo->Map != 18)
		{
			continue;
		}
		...
		if(gMonsterSetBaseChaosCastle.gObjSetPosMonster(index,n) == 0)
		{
			gObjDel(index);
			continue;
		}
...
void CChaosCastle::SetMonsterLevelTwo(CHAOS_CASTLE_LEVEL* lpLevel, int MonsterClass) // OK
{
	for (int n = 0; n < gMonsterSetBaseChaosCastleTwo.m_count; n++)
	{
		if (lpLevel->MaxMonster >= MAX_CC_MONSTER)
		{
			break;
		...
		MONSTER_SET_BASE_INFO_CHAOSCASTLE2* lpInfo = &gMonsterSetBaseChaosCastleTwo.m_MonsterSetBaseInfo[n];

		if (lpInfo->Type != 4 || lpInfo->MonsterClass != MonsterClass || lpInfo->Map != 19)
		{
			continue;
		}
		...
		if (gMonsterSetBaseChaosCastleTwo.gObjSetPosMonster(index, n) == 0)
		{
			gObjDel(index);
			continue;
		}
...
void CChaosCastle::SetMonsterLevelThe(CHAOS_CASTLE_LEVEL* lpLevel, int MonsterClass) // OK
{
	for (int n = 0; n < gMonsterSetBaseChaosCastleThe.m_count; n++)
	{
		if (lpLevel->MaxMonster >= MAX_CC_MONSTER)
		{
			break;
		...
		MONSTER_SET_BASE_INFO_CHAOSCASTLE3* lpInfo = &gMonsterSetBaseChaosCastleThe.m_MonsterSetBaseInfo[n];

		if (lpInfo->Type != 4 || lpInfo->MonsterClass != MonsterClass || lpInfo->Map != 20)
		{
			continue;
		}
		...
		if (gMonsterSetBaseChaosCastleThe.gObjSetPosMonster(index, n) == 0)
		{
			gObjDel(index);
			continue;
		}
...
void CChaosCastle::SetMonsterLevelFor(CHAOS_CASTLE_LEVEL* lpLevel, int MonsterClass) // OK
{
	for (int n = 0; n < gMonsterSetBaseChaosCastleFor.m_count; n++)
	{
		if (lpLevel->MaxMonster >= MAX_CC_MONSTER)
		{
			break;
		...
		MONSTER_SET_BASE_INFO_CHAOSCASTLE4* lpInfo = &gMonsterSetBaseChaosCastleFor.m_MonsterSetBaseInfo[n];

		if (lpInfo->Type != 4 || lpInfo->MonsterClass != MonsterClass || lpInfo->Map != 21)
		{
			continue;
		}
		...
		if (gMonsterSetBaseChaosCastleFor.gObjSetPosMonster(index, n) == 0)
		{
			gObjDel(index);
			continue;
		}
...
void CChaosCastle::SetMonsterLevelFiv(CHAOS_CASTLE_LEVEL* lpLevel, int MonsterClass) // OK
{
	for (int n = 0; n < gMonsterSetBaseChaosCastleFiv.m_count; n++)
	{
		if (lpLevel->MaxMonster >= MAX_CC_MONSTER)
		{
			break;
		...
		MONSTER_SET_BASE_INFO_CHAOSCASTLE5* lpInfo = &gMonsterSetBaseChaosCastleFiv.m_MonsterSetBaseInfo[n];

		if (lpInfo->Type != 4 || lpInfo->MonsterClass != MonsterClass || lpInfo->Map != 22)
		{
			continue;
		}
		...
		if (gMonsterSetBaseChaosCastleFiv.gObjSetPosMonster(index, n) == 0)
		{
			gObjDel(index);
			continue;
		}
...
void CChaosCastle::SetMonsterLevelSix(CHAOS_CASTLE_LEVEL* lpLevel, int MonsterClass) // OK
{
	for (int n = 0; n < gMonsterSetBaseChaosCastleSix.m_count; n++)
	{
		if (lpLevel->MaxMonster >= MAX_CC_MONSTER)
		{
			break;
		...
		MONSTER_SET_BASE_INFO_CHAOSCASTLE6* lpInfo = &gMonsterSetBaseChaosCastleSix.m_MonsterSetBaseInfo[n];

		if (lpInfo->Type != 4 || lpInfo->MonsterClass != MonsterClass || lpInfo->Map != 23)
		{
			continue;
		}
		...
		if (gMonsterSetBaseChaosCastleSix.gObjSetPosMonster(index, n) == 0)
		{
			gObjDel(index);
			continue;
		}
...
void CChaosCastle::SetMonsterLevelSev(CHAOS_CASTLE_LEVEL* lpLevel, int MonsterClass) // OK
{
	for (int n = 0; n < gMonsterSetBaseChaosCastleSev.m_count; n++)
	{
		if (lpLevel->MaxMonster >= MAX_CC_MONSTER)
		{
			break;
		...
		MONSTER_SET_BASE_INFO_CHAOSCASTLE7* lpInfo = &gMonsterSetBaseChaosCastleSev.m_MonsterSetBaseInfo[n];

		if (lpInfo->Type != 4 || lpInfo->MonsterClass != MonsterClass || lpInfo->Map != 53)
		{
			continue;
		}
		...
		if (gMonsterSetBaseChaosCastleSev.gObjSetPosMonster(index, n) == 0)
		{
			gObjDel(index);
			continue;
		}
...
void CChaosCastle::CGChaosCastleEnterRecv(PMSG_CHAOS_CASTLE_ENTER_RECV* lpMsg,int aIndex) // OK
{
	...
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(40));
		return;
	}
