...
void DGCharacterInfoRecv(SDHP_CHARACTER_INFO_RECV* lpMsg) // OK
{
	...
	#if(START_ITEM==1)
	gGiftNew.GiftItem(lpObj);              //ZEUS UP22 CUSTOW BONUS ITENS V2
    #endif

	GDConnectCharacterSend(lpObj->Index);

	g_PersonalShopEx.NPCDialog(lpObj);    //ZEUS UP24 Mercado Negro IN-GAME STORE

	gScriptLoader.OnCharacterEntry(lpObj->Index);   //ZEUS UP24 LUA
	...
	if (info != 0 && lpObj->Socket == INVALID_SOCKET)
	{
		...
		lpObj->IsFakePartyMode = info->PartyMode; //FAKE
		lpObj->ObtainRange = (DWORD)(8);
		lpObj->HuntingRange = (DWORD)(8);

		lpObj->SkillBasicID = info->SkillID;

		if (info->UseBuffs[0] > 0) {
			lpObj->BuffOn = 1;
			lpObj->PartyModeOn = 1;	////FAKEBuff Heal Elf
			lpObj->PartyModeBuffOn = 1; ////FAKEBuff Cho Party
		...
		if (lpObj->IsFakePartyMode == 1) //FAKE
		{
			lpObj->Option |= 1;
			lpObj->Option |= 2;
			lpObj->Option |= 4;
		...
		if (info->TuDongReset == 1)
		{
			lpObj->AutoResetEnable = 1;
			gCustomAttack.DGCustomAttackResumeSaveSend(lpObj->Index);
		}

		LogAdd(LOG_TEST, "[FAKE ONLINE][Obj][%d] AddCharacterInfo (%s) OK", lpObj->Index, lpObj->Name);
	}
#endif


	LogAddConnect(LOG_BLACK,"[Obj][%d] LogInCharacter (%s)",lpObj->Index,lpObj->Name);

	if (gServerInfo.NewsWindowOnLogin == TRUE) //ZEUS UP19 NEW NOTICIAS CHAVE DESLIGAE E LIGAR GS
	{
	g_NewsBoard.OpenMain(lpObj); //ZEUS UP19 NEW NOTICIAS
...
void DGCreateItemRecv(SDHP_CREATE_ITEM_RECV* lpMsg) // OK
{
	if(gObjIsAccountValid(lpMsg->index,lpMsg->account) == 0)
	{
		LogAdd(LOG_RED,"[DGCreateItemRecv] Conta inválida [%d](%s)",lpMsg->index,lpMsg->account);
		CloseClient(lpMsg->index);
		return;
	}
...
void DGPetItemInfoRecv(SDHP_PET_ITEM_INFO_RECV* lpMsg) // OK
{
	if(gObjIsAccountValid(lpMsg->index,lpMsg->account) == 0)
	{
		LogAdd(LOG_RED,"[DGPetItemInfoRecv] Conta inválida [%d](%s)",lpMsg->index,lpMsg->account);
		CloseClient(lpMsg->index);
		return;
	}
...
void DGOptionDataRecv(SDHP_OPTION_DATA_RECV* lpMsg) // OK
{
	if(gObjIsAccountValid(lpMsg->index,lpMsg->account) == 0)
	{
		LogAdd(LOG_RED,"[DGOptionDataRecv] Conta inválida [%d](%s)",lpMsg->index,lpMsg->account);
		CloseClient(lpMsg->index);
		return;
	}
...
void DGCharacterNameCheckRecv(SDHP_CHARACTER_NAME_CHECK_RECV* lpMsg) // OK
{
	...
	if(gObjIsAccountValid(lpMsg->index,lpMsg->account) == 0)
	{
		LogAdd(LOG_RED,"[DGCharacterNameCheckRecv] Conta inválida [%d](%s)",lpMsg->index,lpMsg->account);
		CloseClient(lpMsg->index);
		return;
	}
...
void DGCharacterNameChangeRecv(SDHP_CHARACTER_NAME_CHANGE_RECV* lpMsg) // OK
{
	...
	if(gObjIsAccountValid(lpMsg->index,lpMsg->account) == 0)
	{
		LogAdd(LOG_RED,"[DGCharacterNameChangeRecv] Conta inválida [%d](%s)",lpMsg->index,lpMsg->account);
		CloseClient(lpMsg->index);
		return;
	}
...
void DGGlobalPostRecv(SDHP_GLOBAL_POST_RECV* lpMsg) // OK
{
	...
	switch(lpMsg->type)
	{
		...
		case 3:
			...
			break;
		case 4:
			PostMessage5(lpMsg->name,gMessage.GetMessage(69),lpMsg->message);
			break;
		case 5:
			PostMessage1(lpMsg->name,gMessage.GetMessage(880),lpMsg->message);
			break;
		case 6:
			PostMessage2(lpMsg->name,gMessage.GetMessage(880),lpMsg->message);
			break;
		case 7:
			PostMessage3(lpMsg->name,gMessage.GetMessage(880),lpMsg->message);
			break;
		case 8:
			PostMessage4(lpMsg->name,gMessage.GetMessage(880),lpMsg->message);
			break;
		case 9:
			PostMessage5(lpMsg->name,gMessage.GetMessage(880),lpMsg->message);
			break;
	}
}
...
void DGGlobalWhisperRecv(SDHP_GLOBAL_WHISPER_RECV* lpMsg) // OK
{
	if(gObjIsAccountValid(lpMsg->index,lpMsg->account) == 0)
	{
		LogAdd(LOG_RED,"[DGGlobalWhisperRecv] Conta inválida [%d](%s)",lpMsg->index,lpMsg->account);
		CloseClient(lpMsg->index);
		return;
	}
...
void DGGlobalWhisperEchoRecv(SDHP_GLOBAL_WHISPER_ECHO_RECV* lpMsg) // OK
{
	if(gObjIsAccountValid(lpMsg->index,lpMsg->account) == 0)
	{
		LogAdd(LOG_RED,"[DGGlobalWhisperEchoRecv] Conta inválida [%d](%s)",lpMsg->index,lpMsg->account);
		CloseClient(lpMsg->index);
		return;
	}
...
void GDCreateItemSend(int aIndex, BYTE map,BYTE x,BYTE y,int index,BYTE level,BYTE dur,BYTE Option1,BYTE Option2,BYTE Option3,int LootIndex,BYTE NewOption,BYTE SetOption,BYTE JewelOfHarmonyOption,BYTE ItemOptionEx,BYTE SocketOption[MAX_SOCKET_OPTION],BYTE SocketOptionBonus,DWORD duration) // OK
{
	...
	pMsg.X = x;
	pMsg.Y = y;
	pMsg.Map = ((map==0xEB)?((gEventInventory.IsEventItem(index)==0)?((gMuunSystem.IsMuunItem(index)==0)?((gMuunSystem.IsMuunUtil(index)==0)?map:0xEB):0xEB):0xEC):map);
