...
int APIENTRY WinMain(HINSTANCE hInstance,HINSTANCE hPrevInstance,LPSTR lpCmdLine,int nCmdShow) // OK
{
	...
	CreateThread(NULL, NULL, LPTHREAD_START_ROUTINE(RamFix), NULL, 0, 0); //Zeus UP22 RamGS Fix
	...
	if(WSAStartup(MAKEWORD(2,2),&wsa) == 0)
	{
		...
		{
			LogAdd(LOG_RED,"No foi possvel iniciar o GameServer");
		}
		else
		{
			GameMainInit(hWnd);
			...
			gQueueTimer.CreateTimer(QUEUE_TIMER_LUA_SCRIPT,1000,&QueueTimerCallback);  //ZEUS UP24 LUA TIMER IN-GAME EVENTOS
		}
	}
	else
	{
...
BOOL InitInstance(HINSTANCE hInstance,int nCmdShow) // OK
{
	...
	//hWnd = CreateWindow(szWindowClass,szTitle,WS_OVERLAPPED | WS_MINIMIZEBOX | WS_SYSMENU,CW_USEDEFAULT,0,980,750,0,0,hInstance,0);
	//hWnd = CreateWindow(szWindowClass,szTitle,WS_OVERLAPPEDWINDOW | WS_THICKFRAME,CW_USEDEFAULT,0,950,650,0,0,hInstance,0);
		...
		case WM_COMMAND:
			switch(LOWORD(wParam))
			{
				...
				case IDM_ABOUT:
					...
					break;
				case IDM_EXIT:
					if(MessageBox(0,"Tem certeza de encerrar o GameServer?","Pedir para encerrar o servidor",MB_YESNO | MB_ICONQUESTION) == IDYES)
					{
						gObjAllLogOut();       //ZEUS UP22 ROLLBACK ULTIMO QUE COLOQUEI
				        gObjAllDisconnect();   //ZEUS UP22 ROLLBACK 
						DestroyWindow(hWnd);
					}
					break;
				...
				case IDM_RELOAD_RELOADQUEST:
					...
					break;
				case IDM_RELOAD_RELOADSCRIPT:  //ZEUS UP24 RELOAD LUA
					//Console(LOG_BLUE,"Reload Script");
					gServerInfo.ReadScriptInfo();
					//LogAdd(LOG_BLUE,"[ServerInfo] Script Carregado Com Sucesso");
					break;
				...
				LogAdd(LOG_TEST, "[ FakeOnline ] Disconnect Account");
				}
			}
			...
			break;
		case WM_CLOSE:
			if (MessageBox(0, "Voc tem certeza de terminar GameServer?","ATENO QUER DESLIGAR",MB_OKCANCEL) == IDOK)
			{
				gObjAllLogOut();       //ZEUS UP22 ROLLBACK 
				gObjAllDisconnect();   //ZEUS UP22 ROLLBACK 
				DestroyWindow(hWnd);
			}
			break;
		case WM_PAINT:            //ZEUS UP24 PANTBAR
		    {
			...
			BitBlt(hdc,0,0,1500,80,hMemDC,0,0,SRCCOPY);       //TAMANHO DA DIMENSAO DO BANNER 
...
LRESULT CALLBACK UserOnline(HWND hDlg,UINT message,WPARAM wParam,LPARAM lParam) // OK
{
	...
	switch(message)
	{
		case WM_INITDIALOG:
            ...
            {
                MessageBox(hDlg,
                           "No foi possvel criar a caixa de combinao",
                           "Falha na criao do controle",
                           MB_OK);
                return FALSE;
            }
			...
			case IDC_BUTTONDC:
					...
					for(int n = OBJECT_START_USER; n < MAX_OBJECT ; n++)
					{
						if(gObj[n].Connected >= OBJECT_LOGGED && strcmp(gObj[n].Account,s) == 0 )
						{
							...
							MessageBox(hDlg, "Conta desconectada com sucesso", "confirme", MB_OK);
							break;
						}
					}
...
LRESULT CALLBACK IPBanned(HWND hDlg,UINT message,WPARAM wParam,LPARAM lParam) // OK
{
	switch(message)
	{
		case WM_INITDIALOG:
			...
			hWndComboBox = GetDlgItem(hDlg, IDC_LIST2);
			...
			for ( it = gIpManager.m_IpAddressInfo.begin(); it != gIpManager.m_IpAddressInfo.end(); it++ )
			{
				if(it->second.IpBlocked == 1){
					...
					int index = SendMessage(hWndComboBox, LB_ADDSTRING, 0, reinterpret_cast<LPARAM>((LPCTSTR)fulltext));
					SendMessage(hWndComboBox, LB_SETITEMDATA, (WPARAM) index, (LPARAM) it->second.IpAddress);
				}
			}
			...
			{	
				...
				for ( buscador = gIpManager.m_IpAddressInfo.begin(); buscador != gIpManager.m_IpAddressInfo.end(); buscador++ )
				{
					if(buscador->second.IpAddress == s)
					{
						...
						MessageBox(hDlg, "IP sucessfully unlocked!", "Confirm", MB_OK);
					}
				}
			}
