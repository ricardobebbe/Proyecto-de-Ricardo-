...
bool CItemManager::IsValidItem(int index) // OK
{
	ITEM_INFO ItemInfo;
...
int CItemManager::RepairItem(LPOBJ lpObj,CItem* lpItem,int slot,int type) // OK
{
	...
	if (lpObj->Interface.type == INTERFACE_TRADE)  //ZEUS UP22 FIX DUP ZEN 
	{
		return 0;
	}
...
BYTE CItemManager::MoveItemToMuunInventoryFromInventory(LPOBJ lpObj, BYTE SourceSlot, BYTE TargetSlot, BYTE TargetFlag) // OK
{
	...
	if (gMuunSystem.CheckItemMoveToMuunInventory(lpObj, &lpObj->Inventory[SourceSlot], TargetSlot) == 0)
	{
		return 0xFF;
	}
	...
	if (gMuunSystem.MuunInventoryAddItem(lpObj->Index, lpObj->Inventory[SourceSlot], TargetSlot) == 0xFF)
	{
...
BYTE CItemManager::MoveItemToInventoryFromMuunInventory(LPOBJ lpObj, BYTE SourceSlot, BYTE TargetSlot, BYTE TargetFlag) // OK
{
	//#if(GAMESERVER_UPDATE>=803)
	...
	if (this->CheckItemMoveToInventory(lpObj, &lpObj->MuunInventory[SourceSlot], TargetSlot) == 0)
	{
		return 0xFF;
	}

	if (this->InventoryAddItem(lpObj->Index, lpObj->MuunInventory[SourceSlot], TargetSlot) == 0xFF)
	{
		return 0xFF;
	}
	...
	gMuunSystem.GCMuunItemDeleteSend(lpObj->Index, SourceSlot, 1);
...
void CItemManager::CGItemGetRecv(PMSG_ITEM_GET_RECV* lpMsg,int aIndex) // OK
{
	...
	if(lpObj->Permission[6] != 0)  //ZEUS UP24 LUA
	{
		DataSend(aIndex,(BYTE*)&pMsg,pMsg.header.size);
		//return 0;
		return ;
	...
	if(gScriptLoader.OnUserItemPick(aIndex,index) == 0)    //ZEUS UP24 LUA
	{
		DataSend(aIndex,(BYTE*)&pMsg,pMsg.header.size);
		return ;
	}
...
void CItemManager::CGItemDropRecv(PMSG_ITEM_DROP_RECV* lpMsg,int aIndex) // OK
{
	...
	if(lpObj->Permission[5] != 0)  //ZEUS UP24 LUA
	{
		DataSend(aIndex,(BYTE*)&pMsg,pMsg.header.size);
		return;
	}
	...
	if(gScriptLoader.OnUserItemDrop(aIndex,lpMsg->slot,lpMsg->x,lpMsg->y) == 0)  //ZEUS UP24 LUA 
	{
		DataSend(aIndex,(BYTE*)&pMsg,pMsg.header.size);
		return;
	}
...
void CItemManager::CGItemMoveRecv(PMSG_ITEM_MOVE_RECV* lpMsg,int aIndex) // OK
{
	...
	if(lpObj->Permission[1] != 0)    //ZEUS UP24 LUA
	{
		DataSend(aIndex,(BYTE*)&pMsg,pMsg.header.size);
		return;
	}

	if(gScriptLoader.OnUserItemMove(aIndex,lpMsg->SourceFlag,lpMsg->SourceSlot,lpMsg->TargetFlag,lpMsg->TargetSlot) == 0)  //ZEUS UP24 LUA
	{
		DataSend(aIndex,(BYTE*)&pMsg,pMsg.header.size);
		return;
	}
	...
	//=== New Muun
	else if (lpMsg->SourceFlag == 22 && lpMsg->TargetFlag == 0) // Muun Inventory -> Inventory 
	{
		//LogAdd(LOG_RED, "Muun Inventory -> Inventory  (%d ~ %d)", lpMsg->SourceSlot, lpMsg->TargetSlot);
		if ((pMsg.result = this->MoveItemToInventoryFromMuunInventory(lpObj, lpMsg->SourceSlot, lpMsg->TargetSlot, lpMsg->TargetFlag)) != 0xFF)
		{
			//#if(GAMESERVER_UPDATE>=803)
			this->ItemByteConvert(pMsg.ItemInfo, lpObj->Inventory[lpMsg->TargetSlot]);
			//	#endif
	...
	else if (lpMsg->SourceFlag == 0 && lpMsg->TargetFlag == 22) // Inventory -> Muun Inventory
	{
		//LogAdd(LOG_RED, "Inventory -> Muun Inventory (%d ~ %d)", lpMsg->SourceSlot, lpMsg->TargetSlot);
		if ((pMsg.result = this->MoveItemToMuunInventoryFromInventory(lpObj, lpMsg->SourceSlot, lpMsg->TargetSlot, lpMsg->TargetFlag)) != 0xFF)
		{
			//#if(GAMESERVER_UPDATE>=803)
...
void CItemManager::CGItemUseRecv(PMSG_ITEM_USE_RECV* lpMsg,int aIndex) // OK
{
	...
	{
	    DWORD CurrentTick = GetTickCount();    //ZEUS UP20 POTIONS DALEY
		DWORD Delay = CurrentTick - gObj[aIndex].m_PotionTick;  //ZEUS UP20 POTIONS DALEY
		// ----
		if (Delay < (DWORD)gServerInfo.m_PotionDelay) //ZEUS UP20 POTIONS DALEY
		{
			return;
		}
		else
		{
			if (gObjectManager.CharacterUsePotion(lpObj, lpItem) != 0) //ZEUS UP20 POTIONS DALEY
			{
				lpObj->m_PotionTick = GetTickCount();
				this->DecreaseItemDur(lpObj, lpMsg->SourceSlot, 1); //ZEUS UP20 POTIONS DALEY
			}
		}
	}
...
void CItemManager::CGItemBuyRecv(PMSG_ITEM_BUY_RECV* lpMsg,int aIndex,int ok) // OK
{
	...
	if(lpObj->Permission[3] != 0)  //ZEUS UP 24 LUA
	{
		DataSend(aIndex,(BYTE*)&pMsg,pMsg.header.size);
		return;
	}
...
void CItemManager::CGItemSellRecv(PMSG_ITEM_SELL_RECV* lpMsg,int aIndex) // OK
{
	...
	if(lpObj->Permission[2] != 0)  //ZEUS UP24 LUA
	{
		DataSend(aIndex,(BYTE*)&pMsg,pMsg.header.size);
		return;
	}
