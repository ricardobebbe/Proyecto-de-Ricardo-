...
int LuaGetObjectOfflineFlag(lua_State* L) // OK
{
	...
	if(gObj[aIndex].CustomAttackOffline != 0)
	{
		lua_pushinteger(L,1);
	}
	else if(gObj[aIndex].PShopCustomOffline != 0)
	{
		lua_pushinteger(L,2);
	}
	//else if(gObj[aIndex].Helper.Offline != 0)
	{
		lua_pushinteger(L,3);
	}
//	else
...
int LuaFireworksSend(lua_State* L) // OK
{
	...
	GCServerCommandSend(aIndex,0,aValue,bValue,1);
...
int LuaSQLConnect(lua_State* L) // OK
{
	...
	{
		LogAdd(LOG_RED,"[LuaFunction][SQLConnect] Não foi possível se conectar ao banco de dados [%s][%s][%s]",aString,bString,cString);
		lua_pushinteger(L,0);
	}
	else
	{
