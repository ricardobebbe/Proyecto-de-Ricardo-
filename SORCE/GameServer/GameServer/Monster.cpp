...
bool gObjMonsterRegen(LPOBJ lpObj) // OK
{
	...
	{
		if (gMonsterSetBaseNoria.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
		{
			lpObj->Live = 0;
			lpObj->State = OBJECT_DYING;
			lpObj->DieRegen = 1;
	...
	{
		if (gMonsterSetBaseDungeon.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
		{
			lpObj->Live = 0;
			lpObj->State = OBJECT_DYING;
			lpObj->DieRegen = 1;
	...
	{
		if (gMonsterSetBaseLosttower.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
		{
			lpObj->Live = 0;
			lpObj->State = OBJECT_DYING;
			lpObj->DieRegen = 1;
	...
	{
		if (gMonsterSetBaseArena.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
		{
			lpObj->Live = 0;
			lpObj->State = OBJECT_DYING;
			lpObj->DieRegen = 1;
	...
	{
		if (gMonsterSetBaseAtlans.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
		{
			lpObj->Live = 0;
			lpObj->State = OBJECT_DYING;
			lpObj->DieRegen = 1;
	...
	{
		if (gMonsterSetBaseTarkan.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
		{
			lpObj->Live = 0;
			lpObj->State = OBJECT_DYING;
			lpObj->DieRegen = 1;
	...
	{
		if (gMonsterSetBaseIcarus.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
		{
			lpObj->Live = 0;
			lpObj->State = OBJECT_DYING;
			lpObj->DieRegen = 1;
	...
	else if (lpObj->Map == MAP_KALIMA1)
	{
		if (gMonsterSetBaseKalimaOne.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
		{
			lpObj->Live = 0;
			lpObj->State = OBJECT_DYING;
			lpObj->DieRegen = 1;
	...
	else if (lpObj->Map == MAP_KALIMA2)
	{
		if (gMonsterSetBaseKalimaTwo.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
		{
			lpObj->Live = 0;
			lpObj->State = OBJECT_DYING;
			lpObj->DieRegen = 1;
	...
	else if (lpObj->Map == MAP_KALIMA3)
	{
		if (gMonsterSetBaseKalimaThree.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
		{
			lpObj->Live = 0;
			lpObj->State = OBJECT_DYING;
			lpObj->DieRegen = 1;
	...
	else if (lpObj->Map == MAP_KALIMA4)
	{
		if (gMonsterSetBaseKalimaFour.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
		{
			lpObj->Live = 0;
			lpObj->State = OBJECT_DYING;
			lpObj->DieRegen = 1;
	...
	else if (lpObj->Map == MAP_KALIMA5)
	{
		if (gMonsterSetBaseKalimaFive.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
		{
			lpObj->Live = 0;
			lpObj->State = OBJECT_DYING;
			lpObj->DieRegen = 1;
	...
	else if (lpObj->Map == MAP_KALIMA6)
	{
		if (gMonsterSetBaseKalimaSix.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
		{
			lpObj->Live = 0;
			lpObj->State = OBJECT_DYING;
			lpObj->DieRegen = 1;
	...
	else if (lpObj->Map == MAP_KALIMA7)
	{
		if (gMonsterSetBaseKalimaSeven.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
		{
			lpObj->Live = 0;
			lpObj->State = OBJECT_DYING;
			lpObj->DieRegen = 1;
	...
	{
		if (gMonsterSetBaseCastleSiege.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
		{
			lpObj->Live = 0;
			lpObj->State = OBJECT_DYING;
			lpObj->DieRegen = 1;
	...
	{
		if (gMonsterSetBaseLandOfTrials.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
		{
			lpObj->Live = 0;
			lpObj->State = OBJECT_DYING;
			lpObj->DieRegen = 1;
	...
	{
		if (gMonsterSetBaseCrywolf.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
		{
			lpObj->Live = 0;
			lpObj->State = OBJECT_DYING;
			lpObj->DieRegen = 1;
	...
	{
	if (gMonsterSetBaseAida.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
		{
			lpObj->Live = 0;
			lpObj->State = OBJECT_DYING;
			lpObj->DieRegen = 1;
	...
	{
	//if (gMonsterSetBaseKanturuOne.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
		{
			lpObj->Live = 0;
			lpObj->State = OBJECT_DYING;
			lpObj->DieRegen = 1;
	...
	{
	//if (gMonsterSetBaseKanturuTwo.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
	{
		lpObj->Live = 0;
		lpObj->State = OBJECT_DYING;
		lpObj->DieRegen = 1;
	...
	{
//	if (gKanturuMonsterMng.GetPositionSpotKanturu(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
		{
			lpObj->Live = 0;
			lpObj->State = OBJECT_DYING;
			lpObj->DieRegen = 1;
	...
	{
	if (gMonsterSetBaseBarracks.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
	{
		lpObj->Live = 0;
		lpObj->State = OBJECT_DYING;
		lpObj->DieRegen = 1;
	...
	{
	if (gMonsterSetBaseRefuge.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
	{
		lpObj->Live = 0;
		lpObj->State = OBJECT_DYING;
		lpObj->DieRegen = 1;
	...
	{
	if (gMonsterSetBaseElbeland.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
	{
		lpObj->Live = 0;
		lpObj->State = OBJECT_DYING;
		lpObj->DieRegen = 1;
	...
	{
	if (gMonsterSetBaseSwamp.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
	{
		lpObj->Live = 0;
		lpObj->State = OBJECT_DYING;
		lpObj->DieRegen = 1;
	...
	{
	if (gMonsterSetBaseRaklion.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
	{
		lpObj->Live = 0;
		lpObj->State = OBJECT_DYING;
		lpObj->DieRegen = 1;
	...
	{
	if (gMonsterSetBaseRaklionBoss.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
	{
		lpObj->Live = 0;
		lpObj->State = OBJECT_DYING;
		lpObj->DieRegen = 1;
	...
	{
	if (gMonsterSetBaseSantaTown.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
	{
		lpObj->Live = 0;
		lpObj->State = OBJECT_DYING;
		lpObj->DieRegen = 1;
	...
	{
	if (gMonsterSetBaseLorenMarket.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
	{
		lpObj->Live = 0;
		lpObj->State = OBJECT_DYING;
		lpObj->DieRegen = 1;
	...
	{
	//if (gMonsterSetBaseKarutanOne.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
	{
		lpObj->Live = 0;
		lpObj->State = OBJECT_DYING;
		lpObj->DieRegen = 1;
	...
	{
	//if (gMonsterSetBaseKarutanTwo.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
	{
		lpObj->Live = 0;
		lpObj->State = OBJECT_DYING;
		lpObj->DieRegen = 1;
	...
	else if (lpObj->Map == 84)
	{
		if (gMonsterSetBaseCustomOne.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
		{
			lpObj->Live = 0;
			lpObj->State = OBJECT_DYING;
			lpObj->DieRegen = 1;
	...
	else if (lpObj->Map == 85)
	{
	if (gMonsterSetBaseCustomTwo.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
	{
		lpObj->Live = 0;
		lpObj->State = OBJECT_DYING;
		lpObj->DieRegen = 1;
	...
	else if (lpObj->Map == 86)
	{
	if (gMonsterSetBaseCustomThe.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
	{
		lpObj->Live = 0;
		lpObj->State = OBJECT_DYING;
		lpObj->DieRegen = 1;
	...
	else if (lpObj->Map == 87)
	{
	if (gMonsterSetBaseCustomFor.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
	{
		lpObj->Live = 0;
		lpObj->State = OBJECT_DYING;
		lpObj->DieRegen = 1;
	...
	else if (lpObj->Map == 88)
	{
	if (gMonsterSetBaseCustomFiv.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
	{
		lpObj->Live = 0;
		lpObj->State = OBJECT_DYING;
		lpObj->DieRegen = 1;
	...
	else if (lpObj->Map == 89)
	{
	if (gMonsterSetBaseCustomSix.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
	{
		lpObj->Live = 0;
		lpObj->State = OBJECT_DYING;
		lpObj->DieRegen = 1;
	...
	else if (lpObj->Map == 90)
	{
	if (gMonsterSetBaseCustomSev.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
	{
		lpObj->Live = 0;
		lpObj->State = OBJECT_DYING;
		lpObj->DieRegen = 1;
	...
	else if (lpObj->Map == 95)
	{
	if (gMonsterSetBaseCustomEig.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
	{
		lpObj->Live = 0;
		lpObj->State = OBJECT_DYING;
		lpObj->DieRegen = 1;
	...
	else if (lpObj->Map == 96)
	{
	if (gMonsterSetBaseCustomNin.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
	{
		lpObj->Live = 0;
		lpObj->State = OBJECT_DYING;
		lpObj->DieRegen = 1;
	...
	else if (lpObj->Map == MAP_BLOOD_CASTLE1 )
	{
	if (gMonsterSetBaseBloodCastle.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
	{
		lpObj->Live = 0;
		lpObj->State = OBJECT_DYING;
		lpObj->DieRegen = 1;
	...
	else if (lpObj->Map == MAP_BLOOD_CASTLE2)
	{
	if (gMonsterSetBaseBloodCastleTwo.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
	{
		lpObj->Live = 0;
		lpObj->State = OBJECT_DYING;
		lpObj->DieRegen = 1;
	...
	else if (lpObj->Map == MAP_BLOOD_CASTLE3)
	{
	if (gMonsterSetBaseBloodCastleThe.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
	{
		lpObj->Live = 0;
		lpObj->State = OBJECT_DYING;
		lpObj->DieRegen = 1;
	...
	else if (lpObj->Map == MAP_BLOOD_CASTLE4)
	{
	if (gMonsterSetBaseBloodCastleFor.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
	{
		lpObj->Live = 0;
		lpObj->State = OBJECT_DYING;
		lpObj->DieRegen = 1;
	...
	else if (lpObj->Map == MAP_BLOOD_CASTLE5)
	{
	if (gMonsterSetBaseBloodCastleFiv.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
	{
		lpObj->Live = 0;
		lpObj->State = OBJECT_DYING;
		lpObj->DieRegen = 1;
	...
	else if (lpObj->Map == MAP_BLOOD_CASTLE6)
	{
	if (gMonsterSetBaseBloodCastleSix.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
	{
		lpObj->Live = 0;
		lpObj->State = OBJECT_DYING;
		lpObj->DieRegen = 1;
	...
	else if (lpObj->Map == MAP_BLOOD_CASTLE7)
	{
	if (gMonsterSetBaseBloodCastleSev.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
	{
		lpObj->Live = 0;
		lpObj->State = OBJECT_DYING;
		lpObj->DieRegen = 1;
	...
	else if (lpObj->Map == MAP_BLOOD_CASTLE8)
	{
	if (gMonsterSetBaseBloodCastleEig.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
	{
		lpObj->Live = 0;
		lpObj->State = OBJECT_DYING;
		lpObj->DieRegen = 1;
	...
	else if (lpObj->Map == MAP_DEVIL_SQUARE1)
	{
	if (gMonsterSetBaseDevilSquareOne.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
	{
		lpObj->Live = 0;
		lpObj->State = OBJECT_DYING;
		lpObj->DieRegen = 1;
	...
	else if (lpObj->Map == MAP_DEVIL_SQUARE2)
	{
	if (gMonsterSetBaseDevilSquareTwo.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
	{
		lpObj->Live = 0;
		lpObj->State = OBJECT_DYING;
		lpObj->DieRegen = 1;
	...
	else if (lpObj->Map == MAP_CHAOS_CASTLE1)
	{
	if (gMonsterSetBaseChaosCastle.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
	{
		lpObj->Live = 0;
		lpObj->State = OBJECT_DYING;
		lpObj->DieRegen = 1;
	...
	else if (lpObj->Map == MAP_CHAOS_CASTLE2)
	{
	if (gMonsterSetBaseChaosCastleTwo.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
	{
		lpObj->Live = 0;
		lpObj->State = OBJECT_DYING;
		lpObj->DieRegen = 1;
	...
	else if (lpObj->Map == MAP_CHAOS_CASTLE3)
	{
	if (gMonsterSetBaseChaosCastleThe.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
	{
		lpObj->Live = 0;
		lpObj->State = OBJECT_DYING;
		lpObj->DieRegen = 1;
	...
	else if (lpObj->Map == MAP_CHAOS_CASTLE4)
	{
	if (gMonsterSetBaseChaosCastleFor.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
	{
		lpObj->Live = 0;
		lpObj->State = OBJECT_DYING;
		lpObj->DieRegen = 1;
	...
	else if (lpObj->Map == MAP_CHAOS_CASTLE5)
	{
	if (gMonsterSetBaseChaosCastleFiv.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
	{
		lpObj->Live = 0;
		lpObj->State = OBJECT_DYING;
		lpObj->DieRegen = 1;
	...
	else if (lpObj->Map == MAP_CHAOS_CASTLE6)
	{
	if (gMonsterSetBaseChaosCastleSix.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
	{
		lpObj->Live = 0;
		lpObj->State = OBJECT_DYING;
		lpObj->DieRegen = 1;
	...
	else if (lpObj->Map == MAP_CHAOS_CASTLE7)
	{
	if (gMonsterSetBaseChaosCastleSev.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
	{
		lpObj->Live = 0;
		lpObj->State = OBJECT_DYING;
		lpObj->DieRegen = 1;
	...
	else if (lpObj->Map == MAP_ILLUSION_TEMPLE1)
	{
	if (gMonsterSetBaseIllusionOne.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
	{
		lpObj->Live = 0;
		lpObj->State = OBJECT_DYING;
		lpObj->DieRegen = 1;
	...
	else if (lpObj->Map == MAP_ILLUSION_TEMPLE2)
	{
	if (gMonsterSetBaseIllusionTwo.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
	{
		lpObj->Live = 0;
		lpObj->State = OBJECT_DYING;
		lpObj->DieRegen = 1;
	...
	else if (lpObj->Map == MAP_ILLUSION_TEMPLE3)
	{
	if (gMonsterSetBaseIllusionFor.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
	{
		lpObj->Live = 0;
		lpObj->State = OBJECT_DYING;
		lpObj->DieRegen = 1;
	...
	else if (lpObj->Map == MAP_ILLUSION_TEMPLE4)
	{
	if (gMonsterSetBaseIllusionFor.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
	{
		lpObj->Live = 0;
	...
	else if (lpObj->Map == MAP_ILLUSION_TEMPLE5)
	{
	if (gMonsterSetBaseIllusionFiv.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
	{
		lpObj->Live = 0;
		lpObj->State = OBJECT_DYING;
		lpObj->DieRegen = 1;
	...
	else if (lpObj->Map == MAP_ILLUSION_TEMPLE6)
	{
	if (gMonsterSetBaseIllusionSix.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
	{
		lpObj->Live = 0;
		lpObj->State = OBJECT_DYING;
		lpObj->DieRegen = 1;
	...
	else if (lpObj->Map == MAP_IMPERIAL_GUARDIAN1)
	{
	if (gMonsterSetBaseImperialOne.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
	{
		lpObj->Live = 0;
		lpObj->State = OBJECT_DYING;
		lpObj->DieRegen = 1;
	...
	else if (lpObj->Map == MAP_IMPERIAL_GUARDIAN2)
	{
	if (gMonsterSetBaseImperialTwo.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
	{
		lpObj->Live = 0;
		lpObj->State = OBJECT_DYING;
		lpObj->DieRegen = 1;
	...
	else if (lpObj->Map == MAP_IMPERIAL_GUARDIAN3)
	{
	if (gMonsterSetBaseImperialThe.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
	{
		lpObj->Live = 0;
		lpObj->State = OBJECT_DYING;
		lpObj->DieRegen = 1;
	...
	else if (lpObj->Map == MAP_IMPERIAL_GUARDIAN4)
	{
	if (gMonsterSetBaseImperialFor.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
	{
		lpObj->Live = 0;
		lpObj->State = OBJECT_DYING;
		lpObj->DieRegen = 1;
	...
	{
	if (gMonsterSetBaseInvasion.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
	{
		lpObj->Live = 0;
		lpObj->State = OBJECT_DYING;
		lpObj->DieRegen = 1;
...
bool gObjMonsterMoveCheck(LPOBJ lpObj, int tx, int ty) // OK
{
	...
	if (lpObj->Map == MAP_DUNGEON)
	{
		...
		if (gMonsterSetBaseDungeon.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseDungeon.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == MAP_DEVIAS)
	{
		...
		if (gMonsterSetBaseDevias.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseDevias.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == MAP_NORIA)
	{
		...
		if (gMonsterSetBaseNoria.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseNoria.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == MAP_LOST_TOWER)
	{
		...
		if (gMonsterSetBaseLosttower.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseLosttower.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == MAP_ARENA)
	{
		...
		if (gMonsterSetBaseArena.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseArena.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == MAP_ATLANS)
	{
		...
		if (gMonsterSetBaseAtlans.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseAtlans.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == MAP_TARKAN)
	{
		...
		if (gMonsterSetBaseTarkan.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseTarkan.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == MAP_ICARUS)
	{
		...
		if (gMonsterSetBaseIcarus.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseIcarus.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == MAP_KALIMA1)
	{
		...
		if (gMonsterSetBaseKalimaOne.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseKalimaOne.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == MAP_KALIMA2)
	{
		...
		if (gMonsterSetBaseKalimaTwo.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseKalimaTwo.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == MAP_KALIMA3)
	{
		...
		if (gMonsterSetBaseKalimaThree.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseKalimaThree.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == MAP_KALIMA4)
	{
		...
		if (gMonsterSetBaseKalimaFour.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseKalimaFour.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == MAP_KALIMA5)
	{
		...
		if (gMonsterSetBaseKalimaFive.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseKalimaFive.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == MAP_KALIMA6)
	{
		...
		if (gMonsterSetBaseKalimaSix.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseKalimaSix.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == MAP_KALIMA7)
	{
		...
		if (gMonsterSetBaseKalimaSeven.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseKalimaSeven.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == MAP_CASTLE_SIEGE)
	{
		...
		if (gMonsterSetBaseCastleSiege.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseCastleSiege.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == MAP_LAND_OF_TRIALS)
	{
		...
		if (gMonsterSetBaseLandOfTrials.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseLandOfTrials.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == MAP_CRYWOLF)
	{
		...
		if (gMonsterSetBaseCrywolf.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseCrywolf.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == MAP_AIDA)
	{
		...
		if (gMonsterSetBaseAida.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseAida.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == MAP_KANTURU1)
	{
		...
		//if (gMonsterSetBaseKanturuOne.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		//if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseKanturuOne.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == MAP_KANTURU2)
	{
		...
		if (gMonsterSetBaseKanturuTwo.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseKanturuTwo.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == MAP_BARRACKS)
	{
		...
		if (gMonsterSetBaseBarracks.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseBarracks.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == MAP_REFUGE)
	{
		...
		if (gMonsterSetBaseRefuge.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseRefuge.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == MAP_ELBELAND)
	{
		...
		if (gMonsterSetBaseElbeland.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseElbeland.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == MAP_SWAMP_OF_CALMNESS)
	{
		...
		if (gMonsterSetBaseSwamp.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseSwamp.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == MAP_RAKLION1)
	{
		...
		if (gMonsterSetBaseRaklion.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseRaklion.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == MAP_RAKLION2)
	{
		...
		if (gMonsterSetBaseRaklionBoss.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseRaklionBoss.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == MAP_SANTA_TOWN)
	{
		...
		if (gMonsterSetBaseSantaTown.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseSantaTown.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == MAP_LOREN_MARKET)
	{
		...
		if (gMonsterSetBaseLorenMarket.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseLorenMarket.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
...
//		if (gMonsterSetBaseKarutanOne.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

//		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseKarutanOne.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == MAP_KARUTAN2)
	{
		...
		//if (gMonsterSetBaseKarutanTwo.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		//if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseKarutanTwo.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == 84)
	{
		...
		if (gMonsterSetBaseCustomOne.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseCustomOne.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == 85)
	{
		...
		if (gMonsterSetBaseCustomTwo.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseCustomTwo.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == 86)
	{
		...
		if (gMonsterSetBaseCustomThe.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseCustomThe.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == 87)
	{
		...
		if (gMonsterSetBaseCustomFor.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseCustomFor.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == 88)
	{
		...
		if (gMonsterSetBaseCustomFiv.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseCustomFiv.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == 89)
	{
		...
		if (gMonsterSetBaseCustomSix.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseCustomSix.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == 90)
	{
		...
		if (gMonsterSetBaseCustomSev.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseCustomSev.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == 95)
	{
		...
		if (gMonsterSetBaseCustomEig.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseCustomEig.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == 96)
	{
		...
		if (gMonsterSetBaseCustomNin.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseCustomNin.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == MAP_BLOOD_CASTLE1)
	{
		...
		if (gMonsterSetBaseBloodCastle.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseBloodCastle.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == MAP_BLOOD_CASTLE2)
	{
		...
		if (gMonsterSetBaseBloodCastleTwo.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseBloodCastleTwo.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == MAP_BLOOD_CASTLE3)
	{
		...
		if (gMonsterSetBaseBloodCastleThe.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseBloodCastleThe.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == MAP_BLOOD_CASTLE4)
	{
		...
		if (gMonsterSetBaseBloodCastleFor.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseBloodCastleFor.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == MAP_BLOOD_CASTLE5)
	{
		...
		if (gMonsterSetBaseBloodCastleFiv.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseBloodCastleFiv.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == MAP_BLOOD_CASTLE6)
	{
		...
		if (gMonsterSetBaseBloodCastleSix.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseBloodCastleSix.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == MAP_BLOOD_CASTLE7)
	{
		...
		if (gMonsterSetBaseBloodCastleSev.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseBloodCastleSev.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == MAP_BLOOD_CASTLE8)
	{
		...
		if (gMonsterSetBaseBloodCastleEig.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseBloodCastleEig.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == MAP_DEVIL_SQUARE1)
	{
		...
		if (gMonsterSetBaseDevilSquareOne.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseDevilSquareOne.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == MAP_DEVIL_SQUARE2)
	{
		...
		if (gMonsterSetBaseDevilSquareTwo.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseDevilSquareTwo.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == MAP_CHAOS_CASTLE1)
	{
		...
		if (gMonsterSetBaseChaosCastle.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseChaosCastle.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == MAP_CHAOS_CASTLE2)
	{
		...
		if (gMonsterSetBaseChaosCastleTwo.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseChaosCastleTwo.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == MAP_CHAOS_CASTLE3)
	{
		...
		if (gMonsterSetBaseChaosCastleThe.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseChaosCastleThe.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == MAP_CHAOS_CASTLE4)
	{
		...
		if (gMonsterSetBaseChaosCastleFor.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseChaosCastleFor.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == MAP_CHAOS_CASTLE5)
	{
		...
		if (gMonsterSetBaseChaosCastleFiv.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseChaosCastleFiv.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == MAP_CHAOS_CASTLE6)
	{
		...
		if (gMonsterSetBaseChaosCastleSix.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseChaosCastleSix.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == MAP_CHAOS_CASTLE7)
	{
		...
		if (gMonsterSetBaseChaosCastleSev.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseChaosCastleSev.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == MAP_ILLUSION_TEMPLE1)
	{
		...
		if (gMonsterSetBaseIllusionOne.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseIllusionOne.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == MAP_ILLUSION_TEMPLE2)
	{
		...
		if (gMonsterSetBaseIllusionTwo.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseIllusionTwo.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == MAP_ILLUSION_TEMPLE3)
	{
		...
		if (gMonsterSetBaseIllusionThe.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseIllusionThe.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == MAP_ILLUSION_TEMPLE4)
	{
		...
		if (gMonsterSetBaseIllusionFor.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseIllusionFor.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == MAP_ILLUSION_TEMPLE5)
	{
		...
		if (gMonsterSetBaseIllusionFiv.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseIllusionFiv.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == MAP_ILLUSION_TEMPLE6)
	{
		...
		if (gMonsterSetBaseIllusionSix.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseIllusionSix.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == MAP_IMPERIAL_GUARDIAN1)
	{
		...
		if (gMonsterSetBaseImperialOne.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseImperialOne.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == MAP_IMPERIAL_GUARDIAN2)
	{
		...
		if (gMonsterSetBaseImperialTwo.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseImperialTwo.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == MAP_IMPERIAL_GUARDIAN3)
	{
		...
		if (gMonsterSetBaseImperialThe.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseImperialThe.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	if (lpObj->Map == MAP_IMPERIAL_GUARDIAN4)
	{
		...
		if (gMonsterSetBaseImperialFor.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseImperialFor.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
	...
	else
	{
		...
		if (gMonsterSetBaseInvasion.m_MonsterSetBaseInfo[lpObj->PosNum].Dis == 0)
		{
			return 0;
		}

		if (sqrt((pow((float)(tx - lpObj->StartX), (float)2) + pow((float)(ty - lpObj->StartY), (float)2))) > gMonsterSetBaseInvasion.m_MonsterSetBaseInfo[lpObj->PosNum].Dis)
		{
			return 0;
		}
	}
...
/*void gObjMonsterUpdateProc(LPOBJ lpObj) //ZEUS UP 24 LUA
{
