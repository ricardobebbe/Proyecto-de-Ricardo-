// MonsterSetBase.cpp: implementation of the CMonsterSetBaseArena class.
//
//////////////////////////////////////////////////////////////////////
...
CMonsterSetBaseArena gMonsterSetBaseArena;
//////////////////////////////////////////////////////////////////////
// Construction/Destruction
//////////////////////////////////////////////////////////////////////

CMonsterSetBaseArena::CMonsterSetBaseArena() // OK
{
	this->m_count = 0;
}
...
void CMonsterSetBaseArena::Load(char* path) // OK
{
	...
	try
	{
		while (true)
		{
			...
			while (true)
			{
				...
				MONSTER_SET_BASE_INFO_ARENA info;
...
void CMonsterSetBaseArena::SetInfo(MONSTER_SET_BASE_INFO_ARENA info) // OK
{
...
bool CMonsterSetBaseArena::GetPosition(int index, short map, short* ox, short* oy) // OK
{
	...
	MONSTER_SET_BASE_INFO_ARENA* lpInfo = &this->m_MonsterSetBaseInfo[index];
...
bool CMonsterSetBaseArena::GetBoxPosition(int map, int x, int y, int tx, int ty, short* ox, short* oy) // OK
{
	for (int n = 0; n < 100; n++)
	{
		int subx = tx - x;
...
void CMonsterSetBaseArena::SetBoxPosition(int index, int map, int x, int y, int tx, int ty) // OK
{
	if (index < 0 || index >= MAX_MSB_MONSTER)
	{
		return;
...
void CMonsterSetBaseArena::SetMonsterDataArena() // OK
{
	for (int n = 0; n < gMonsterSetBaseArena.m_count; n++)
	{
		MONSTER_SET_BASE_INFO_ARENA* lpInfo = &gMonsterSetBaseArena.m_MonsterSetBaseInfo[n];
...
bool CMonsterSetBaseArena::gObjSetPosMonster(int aIndex, int number) // OK
{
	...
	MONSTER_SET_BASE_INFO_ARENA* lpInfo = &gMonsterSetBaseArena.m_MonsterSetBaseInfo[number];

	short x, y;

	if (gMonsterSetBaseArena.GetPosition(number, lpInfo->Map, &x, &y) == 0)
	{
		return 0;
	}
