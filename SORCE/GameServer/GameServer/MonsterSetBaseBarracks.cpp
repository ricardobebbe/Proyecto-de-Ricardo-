// MonsterSetBase.cpp: implementation of the CMonsterSetBaseBarracks class.
//
//////////////////////////////////////////////////////////////////////
...
CMonsterSetBaseBarracks gMonsterSetBaseBarracks;
//////////////////////////////////////////////////////////////////////
// Construction/Destruction
//////////////////////////////////////////////////////////////////////

CMonsterSetBaseBarracks::CMonsterSetBaseBarracks() // OK
{
	this->m_count = 0;
}
...
void CMonsterSetBaseBarracks::Load(char* path) // OK
{
	...
	try
	{
		while (true)
		{
			...
			while (true)
			{
				...
				MONSTER_SET_BASE_INFO_BARRACKS info;
...
void CMonsterSetBaseBarracks::SetInfo(MONSTER_SET_BASE_INFO_BARRACKS info) // OK
{
...
bool CMonsterSetBaseBarracks::GetPosition(int index, short map, short* ox, short* oy) // OK
{
	...
	MONSTER_SET_BASE_INFO_BARRACKS* lpInfo = &this->m_MonsterSetBaseInfo[index];
...
bool CMonsterSetBaseBarracks::GetBoxPosition(int map, int x, int y, int tx, int ty, short* ox, short* oy) // OK
{
	for (int n = 0; n < 100; n++)
	{
		int subx = tx - x;
...
void CMonsterSetBaseBarracks::SetBoxPosition(int index, int map, int x, int y, int tx, int ty) // OK
{
	if (index < 0 || index >= MAX_MSB_MONSTER)
	{
		return;
...
void CMonsterSetBaseBarracks::SetMonsterDataBarracks() // OK
{
	for (int n = 0; n < gMonsterSetBaseBarracks.m_count; n++)
	{
		MONSTER_SET_BASE_INFO_BARRACKS* lpInfo = &gMonsterSetBaseBarracks.m_MonsterSetBaseInfo[n];
...
bool CMonsterSetBaseBarracks::gObjSetPosMonster(int aIndex, int number) // OK
{
	...
	MONSTER_SET_BASE_INFO_BARRACKS* lpInfo = &gMonsterSetBaseBarracks.m_MonsterSetBaseInfo[number];

	short x, y;

	if (gMonsterSetBaseBarracks.GetPosition(number, lpInfo->Map, &x, &y) == 0)
	{
		return 0;
	}
