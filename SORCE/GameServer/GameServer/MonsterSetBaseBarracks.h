...
struct MONSTER_SET_BASE_INFO_BARRACKS
{
	int Type;
	int MonsterClass;
	int Map;
...
class CMonsterSetBaseBarracks
{
public:
	CMonsterSetBaseBarracks();
	virtual ~CMonsterSetBaseBarracks();
	void Load(char* path);
	void SetInfo(MONSTER_SET_BASE_INFO_BARRACKS info);
	...
	void SetMonsterDataBarracks();
	bool gObjSetPosMonster(int aIndex, int number);
public:
	MONSTER_SET_BASE_INFO_BARRACKS m_MonsterSetBaseInfo[MAX_MSB_MONSTER];
	MONSTER_SET_BASE_INFO_BARRACKS m_Mp[OBJ_MAXMONSTER];//MC
	int m_count;
};

extern CMonsterSetBaseBarracks gMonsterSetBaseBarracks;
