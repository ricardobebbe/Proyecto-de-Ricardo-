// MonsterSetBase.cpp: implementation of the CMonsterSetBaseCastleSiege class.
//
//////////////////////////////////////////////////////////////////////
...
CMonsterSetBaseCastleSiege gMonsterSetBaseCastleSiege;
//////////////////////////////////////////////////////////////////////
// Construction/Destruction
//////////////////////////////////////////////////////////////////////

CMonsterSetBaseCastleSiege::CMonsterSetBaseCastleSiege() // OK
{
	this->m_count = 0;
}
...
void CMonsterSetBaseCastleSiege::Load(char* path) // OK
{
	...
	try
	{
		while (true)
		{
			...
			while (true)
			{
				...
				MONSTER_SET_BASE_INFO_CASTLESIEGE info;
...
void CMonsterSetBaseCastleSiege::SetInfo(MONSTER_SET_BASE_INFO_CASTLESIEGE info) // OK
{
...
bool CMonsterSetBaseCastleSiege::GetPosition(int index, short map, short* ox, short* oy) // OK
{
	...
	MONSTER_SET_BASE_INFO_CASTLESIEGE* lpInfo = &this->m_MonsterSetBaseInfo[index];
...
bool CMonsterSetBaseCastleSiege::GetBoxPosition(int map, int x, int y, int tx, int ty, short* ox, short* oy) // OK
{
	for (int n = 0; n < 100; n++)
	{
		int subx = tx - x;
...
void CMonsterSetBaseCastleSiege::SetBoxPosition(int index, int map, int x, int y, int tx, int ty) // OK
{
	if (index < 0 || index >= MAX_MSB_MONSTER)
	{
		return;
...
void CMonsterSetBaseCastleSiege::SetMonsterDataCastleSiege() // OK
{
	for (int n = 0; n < gMonsterSetBaseCastleSiege.m_count; n++)
	{
		MONSTER_SET_BASE_INFO_CASTLESIEGE* lpInfo = &gMonsterSetBaseCastleSiege.m_MonsterSetBaseInfo[n];
...
bool CMonsterSetBaseCastleSiege::gObjSetPosMonster(int aIndex, int number) // OK
{
	...
	MONSTER_SET_BASE_INFO_CASTLESIEGE* lpInfo = &gMonsterSetBaseCastleSiege.m_MonsterSetBaseInfo[number];

	short x, y;

	if (gMonsterSetBaseCastleSiege.GetPosition(number, lpInfo->Map, &x, &y) == 0)
	{
		return 0;
	}
