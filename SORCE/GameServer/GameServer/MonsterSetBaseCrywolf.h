...
struct MONSTER_SET_BASE_INFO_CRYWOLF
{
	int Type;
	int MonsterClass;
	int Map;
...
class CMonsterSetBaseCrywolf
{
public:
	CMonsterSetBaseCrywolf();
	virtual ~CMonsterSetBaseCrywolf();
	void Load(char* path);
	void SetInfo(MONSTER_SET_BASE_INFO_CRYWOLF info);
	...
	void SetMonsterDataCrywolf();
	bool gObjSetPosMonster(int aIndex, int number);
public:
	MONSTER_SET_BASE_INFO_CRYWOLF m_MonsterSetBaseInfo[MAX_MSB_MONSTER];
	MONSTER_SET_BASE_INFO_CRYWOLF m_Mp[OBJ_MAXMONSTER];//MC
	int m_count;
};

extern CMonsterSetBaseCrywolf gMonsterSetBaseCrywolf;
