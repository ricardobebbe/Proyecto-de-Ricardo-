// MonsterSetBase.cpp: implementation of the CMonsterSetBaseCustomEig class.
//
//////////////////////////////////////////////////////////////////////
...
CMonsterSetBaseCustomEig gMonsterSetBaseCustomEig;
//////////////////////////////////////////////////////////////////////
// Construction/Destruction
//////////////////////////////////////////////////////////////////////

CMonsterSetBaseCustomEig::CMonsterSetBaseCustomEig() // OK
{
	this->m_count = 0;
}
...
void CMonsterSetBaseCustomEig::Load(char* path) // OK
{
	...
	try
	{
		while (true)
		{
			...
			while (true)
			{
				...
				MONSTER_SET_BASE_INFO_CUSTOM8 info;
...
void CMonsterSetBaseCustomEig::SetInfo(MONSTER_SET_BASE_INFO_CUSTOM8 info) // OK
{
...
bool CMonsterSetBaseCustomEig::GetPosition(int index, short map, short* ox, short* oy) // OK
{
	...
	MONSTER_SET_BASE_INFO_CUSTOM8* lpInfo = &this->m_MonsterSetBaseInfo[index];
...
bool CMonsterSetBaseCustomEig::GetBoxPosition(int map, int x, int y, int tx, int ty, short* ox, short* oy) // OK
{
	for (int n = 0; n < 100; n++)
	{
		int subx = tx - x;
...
void CMonsterSetBaseCustomEig::SetBoxPosition(int index, int map, int x, int y, int tx, int ty) // OK
{
	if (index < 0 || index >= MAX_MSB_MONSTER)
	{
		return;
...
void CMonsterSetBaseCustomEig::SetMonsterDataCustomEig() // OK
{
	for (int n = 0; n < gMonsterSetBaseCustomEig.m_count; n++)
	{
		MONSTER_SET_BASE_INFO_CUSTOM8* lpInfo = &gMonsterSetBaseCustomEig.m_MonsterSetBaseInfo[n];
...
bool CMonsterSetBaseCustomEig::gObjSetPosMonster(int aIndex, int number) // OK
{
	...
	MONSTER_SET_BASE_INFO_CUSTOM8* lpInfo = &gMonsterSetBaseCustomEig.m_MonsterSetBaseInfo[number];

	short x, y;

	if (gMonsterSetBaseCustomEig.GetPosition(number, lpInfo->Map, &x, &y) == 0)
	{
		return 0;
	}
