...
struct MONSTER_SET_BASE_INFO_CUSTOM8
{
	int Type;
	int MonsterClass;
	int Map;
...
class CMonsterSetBaseCustomEig
{
public:
	CMonsterSetBaseCustomEig();
	virtual ~CMonsterSetBaseCustomEig();
	void Load(char* path);
	void SetInfo(MONSTER_SET_BASE_INFO_CUSTOM8 info);
	...
	void SetMonsterDataCustomEig();
	bool gObjSetPosMonster(int aIndex, int number);
public:
	MONSTER_SET_BASE_INFO_CUSTOM8 m_MonsterSetBaseInfo[MAX_MSB_MONSTER];
	MONSTER_SET_BASE_INFO_CUSTOM8 m_Mp[OBJ_MAXMONSTER];//MC
	int m_count;
};

extern CMonsterSetBaseCustomEig gMonsterSetBaseCustomEig;
