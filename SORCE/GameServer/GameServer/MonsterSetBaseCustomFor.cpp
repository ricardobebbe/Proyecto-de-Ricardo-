// MonsterSetBase.cpp: implementation of the CMonsterSetBaseCustomFor class.
//
//////////////////////////////////////////////////////////////////////
...
CMonsterSetBaseCustomFor gMonsterSetBaseCustomFor;
//////////////////////////////////////////////////////////////////////
// Construction/Destruction
//////////////////////////////////////////////////////////////////////

CMonsterSetBaseCustomFor::CMonsterSetBaseCustomFor() // OK
{
	this->m_count = 0;
}
...
void CMonsterSetBaseCustomFor::Load(char* path) // OK
{
	...
	try
	{
		while (true)
		{
			...
			while (true)
			{
				...
				MONSTER_SET_BASE_INFO_CUSTOM4 info;
...
void CMonsterSetBaseCustomFor::SetInfo(MONSTER_SET_BASE_INFO_CUSTOM4 info) // OK
{
...
bool CMonsterSetBaseCustomFor::GetPosition(int index, short map, short* ox, short* oy) // OK
{
	...
	MONSTER_SET_BASE_INFO_CUSTOM4* lpInfo = &this->m_MonsterSetBaseInfo[index];
...
bool CMonsterSetBaseCustomFor::GetBoxPosition(int map, int x, int y, int tx, int ty, short* ox, short* oy) // OK
{
	for (int n = 0; n < 100; n++)
	{
		int subx = tx - x;
...
void CMonsterSetBaseCustomFor::SetBoxPosition(int index, int map, int x, int y, int tx, int ty) // OK
{
	if (index < 0 || index >= MAX_MSB_MONSTER)
	{
		return;
...
void CMonsterSetBaseCustomFor::SetMonsterDataCustomFor() // OK
{
	for (int n = 0; n < gMonsterSetBaseCustomFor.m_count; n++)
	{
		MONSTER_SET_BASE_INFO_CUSTOM4* lpInfo = &gMonsterSetBaseCustomFor.m_MonsterSetBaseInfo[n];
...
bool CMonsterSetBaseCustomFor::gObjSetPosMonster(int aIndex, int number) // OK
{
	...
	MONSTER_SET_BASE_INFO_CUSTOM4* lpInfo = &gMonsterSetBaseCustomFor.m_MonsterSetBaseInfo[number];

	short x, y;

	if (gMonsterSetBaseCustomFor.GetPosition(number, lpInfo->Map, &x, &y) == 0)
	{
		return 0;
	}
