...
struct MONSTER_SET_BASE_INFO_CUSTOM4
{
	int Type;
	int MonsterClass;
	int Map;
...
class CMonsterSetBaseCustomFor
{
public:
	CMonsterSetBaseCustomFor();
	virtual ~CMonsterSetBaseCustomFor();
	void Load(char* path);
	void SetInfo(MONSTER_SET_BASE_INFO_CUSTOM4 info);
	...
	void SetMonsterDataCustomFor();
	bool gObjSetPosMonster(int aIndex, int number);
public:
	MONSTER_SET_BASE_INFO_CUSTOM4 m_MonsterSetBaseInfo[MAX_MSB_MONSTER];
	MONSTER_SET_BASE_INFO_CUSTOM4 m_Mp[OBJ_MAXMONSTER];//MC
	int m_count;
};

extern CMonsterSetBaseCustomFor gMonsterSetBaseCustomFor;
