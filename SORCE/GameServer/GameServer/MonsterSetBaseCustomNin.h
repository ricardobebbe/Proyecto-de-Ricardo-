...
struct MONSTER_SET_BASE_INFO_CUSTOM9
{
	int Type;
	int MonsterClass;
	int Map;
...
class CMonsterSetBaseCustomNin
{
public:
	CMonsterSetBaseCustomNin();
	virtual ~CMonsterSetBaseCustomNin();
	void Load(char* path);
	void SetInfo(MONSTER_SET_BASE_INFO_CUSTOM9 info);
	...
	void SetMonsterDataCustomNin();
	bool gObjSetPosMonster(int aIndex, int number);
public:
	MONSTER_SET_BASE_INFO_CUSTOM9 m_MonsterSetBaseInfo[MAX_MSB_MONSTER];
	MONSTER_SET_BASE_INFO_CUSTOM9 m_Mp[OBJ_MAXMONSTER];//MC
	int m_count;
};

extern CMonsterSetBaseCustomNin gMonsterSetBaseCustomNin;
