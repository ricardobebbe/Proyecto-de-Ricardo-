...
struct MONSTER_SET_BASE_INFO_CUSTOM1
{
	int Type;
	int MonsterClass;
	int Map;
...
class CMonsterSetBaseCustomOne
{
public:
	CMonsterSetBaseCustomOne();
	virtual ~CMonsterSetBaseCustomOne();
	void Load(char* path);
	void SetInfo(MONSTER_SET_BASE_INFO_CUSTOM1 info);
	...
	void SetMonsterDataCustomOne();
	bool gObjSetPosMonster(int aIndex, int number);
public:
	MONSTER_SET_BASE_INFO_CUSTOM1 m_MonsterSetBaseInfo[MAX_MSB_MONSTER];
	MONSTER_SET_BASE_INFO_CUSTOM1 m_Mp[OBJ_MAXMONSTER];//MC
	int m_count;
};

extern CMonsterSetBaseCustomOne gMonsterSetBaseCustomOne;
