// MonsterSetBase.cpp: implementation of the CMonsterSetBaseCustomSev class.
//
//////////////////////////////////////////////////////////////////////
...
CMonsterSetBaseCustomSev gMonsterSetBaseCustomSev;
//////////////////////////////////////////////////////////////////////
// Construction/Destruction
//////////////////////////////////////////////////////////////////////

CMonsterSetBaseCustomSev::CMonsterSetBaseCustomSev() // OK
{
	this->m_count = 0;
}
...
void CMonsterSetBaseCustomSev::Load(char* path) // OK
{
	...
	try
	{
		while (true)
		{
			...
			while (true)
			{
				...
				MONSTER_SET_BASE_INFO_CUSTOM7 info;
...
void CMonsterSetBaseCustomSev::SetInfo(MONSTER_SET_BASE_INFO_CUSTOM7 info) // OK
{
...
bool CMonsterSetBaseCustomSev::GetPosition(int index, short map, short* ox, short* oy) // OK
{
	...
	MONSTER_SET_BASE_INFO_CUSTOM7* lpInfo = &this->m_MonsterSetBaseInfo[index];
...
bool CMonsterSetBaseCustomSev::GetBoxPosition(int map, int x, int y, int tx, int ty, short* ox, short* oy) // OK
{
	for (int n = 0; n < 100; n++)
	{
		int subx = tx - x;
...
void CMonsterSetBaseCustomSev::SetBoxPosition(int index, int map, int x, int y, int tx, int ty) // OK
{
	if (index < 0 || index >= MAX_MSB_MONSTER)
	{
		return;
...
void CMonsterSetBaseCustomSev::SetMonsterDataCustomSev() // OK
{
	for (int n = 0; n < gMonsterSetBaseCustomSev.m_count; n++)
	{
		MONSTER_SET_BASE_INFO_CUSTOM7* lpInfo = &gMonsterSetBaseCustomSev.m_MonsterSetBaseInfo[n];
...
bool CMonsterSetBaseCustomSev::gObjSetPosMonster(int aIndex, int number) // OK
{
	...
	MONSTER_SET_BASE_INFO_CUSTOM7* lpInfo = &gMonsterSetBaseCustomSev.m_MonsterSetBaseInfo[number];

	short x, y;

	if (gMonsterSetBaseCustomSev.GetPosition(number, lpInfo->Map, &x, &y) == 0)
	{
		return 0;
	}
