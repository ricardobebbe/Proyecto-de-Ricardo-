...
struct MONSTER_SET_BASE_INFO_CUSTOM7
{
	int Type;
	int MonsterClass;
	int Map;
...
class CMonsterSetBaseCustomSev
{
public:
	CMonsterSetBaseCustomSev();
	virtual ~CMonsterSetBaseCustomSev();
	void Load(char* path);
	void SetInfo(MONSTER_SET_BASE_INFO_CUSTOM7 info);
	...
	void SetMonsterDataCustomSev();
	bool gObjSetPosMonster(int aIndex, int number);
public:
	MONSTER_SET_BASE_INFO_CUSTOM7 m_MonsterSetBaseInfo[MAX_MSB_MONSTER];
	MONSTER_SET_BASE_INFO_CUSTOM7 m_Mp[OBJ_MAXMONSTER];//MC
	int m_count;
};

extern CMonsterSetBaseCustomSev gMonsterSetBaseCustomSev;
