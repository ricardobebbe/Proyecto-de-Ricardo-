// MonsterSetBase.cpp: implementation of the CMonsterSetBaseCustomSix class.
//
//////////////////////////////////////////////////////////////////////
...
CMonsterSetBaseCustomSix gMonsterSetBaseCustomSix;
//////////////////////////////////////////////////////////////////////
// Construction/Destruction
//////////////////////////////////////////////////////////////////////

CMonsterSetBaseCustomSix::CMonsterSetBaseCustomSix() // OK
{
	this->m_count = 0;
}
...
void CMonsterSetBaseCustomSix::Load(char* path) // OK
{
	...
	try
	{
		while (true)
		{
			...
			while (true)
			{
				...
				MONSTER_SET_BASE_INFO_CUSTOM6 info;
...
void CMonsterSetBaseCustomSix::SetInfo(MONSTER_SET_BASE_INFO_CUSTOM6 info) // OK
{
...
bool CMonsterSetBaseCustomSix::GetPosition(int index, short map, short* ox, short* oy) // OK
{
	...
	MONSTER_SET_BASE_INFO_CUSTOM6* lpInfo = &this->m_MonsterSetBaseInfo[index];
...
bool CMonsterSetBaseCustomSix::GetBoxPosition(int map, int x, int y, int tx, int ty, short* ox, short* oy) // OK
{
	for (int n = 0; n < 100; n++)
	{
		int subx = tx - x;
...
void CMonsterSetBaseCustomSix::SetBoxPosition(int index, int map, int x, int y, int tx, int ty) // OK
{
	if (index < 0 || index >= MAX_MSB_MONSTER)
	{
		return;
...
void CMonsterSetBaseCustomSix::SetMonsterDataCustomSix() // OK
{
	for (int n = 0; n < gMonsterSetBaseCustomSix.m_count; n++)
	{
		MONSTER_SET_BASE_INFO_CUSTOM6* lpInfo = &gMonsterSetBaseCustomSix.m_MonsterSetBaseInfo[n];
...
bool CMonsterSetBaseCustomSix::gObjSetPosMonster(int aIndex, int number) // OK
{
	...
	MONSTER_SET_BASE_INFO_CUSTOM6* lpInfo = &gMonsterSetBaseCustomSix.m_MonsterSetBaseInfo[number];

	short x, y;

	if (gMonsterSetBaseCustomSix.GetPosition(number, lpInfo->Map, &x, &y) == 0)
	{
		return 0;
	}
