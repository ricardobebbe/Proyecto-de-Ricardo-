...
struct MONSTER_SET_BASE_INFO_CUSTOM6
{
	int Type;
	int MonsterClass;
	int Map;
...
class CMonsterSetBaseCustomSix
{
public:
	CMonsterSetBaseCustomSix();
	virtual ~CMonsterSetBaseCustomSix();
	void Load(char* path);
	void SetInfo(MONSTER_SET_BASE_INFO_CUSTOM6 info);
	...
	void SetMonsterDataCustomSix();
	bool gObjSetPosMonster(int aIndex, int number);
public:
	MONSTER_SET_BASE_INFO_CUSTOM6 m_MonsterSetBaseInfo[MAX_MSB_MONSTER];
	MONSTER_SET_BASE_INFO_CUSTOM6 m_Mp[OBJ_MAXMONSTER];//MC
	int m_count;
};

extern CMonsterSetBaseCustomSix gMonsterSetBaseCustomSix;
