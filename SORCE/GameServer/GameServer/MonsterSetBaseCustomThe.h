...
struct MONSTER_SET_BASE_INFO_CUSTOM3
{
	int Type;
	int MonsterClass;
	int Map;
...
class CMonsterSetBaseCustomThe
{
public:
	CMonsterSetBaseCustomThe();
	virtual ~CMonsterSetBaseCustomThe();
	void Load(char* path);
	void SetInfo(MONSTER_SET_BASE_INFO_CUSTOM3 info);
	...
	void SetMonsterDataCustomThe();
	bool gObjSetPosMonster(int aIndex, int number);
public:
	MONSTER_SET_BASE_INFO_CUSTOM3 m_MonsterSetBaseInfo[MAX_MSB_MONSTER];
	MONSTER_SET_BASE_INFO_CUSTOM3 m_Mp[OBJ_MAXMONSTER];//MC
	int m_count;
};

extern CMonsterSetBaseCustomThe gMonsterSetBaseCustomThe;
