...
struct MONSTER_SET_BASE_INFO_CUSTOM2
{
	int Type;
	int MonsterClass;
	int Map;
...
class CMonsterSetBaseCustomTwo
{
public:
	CMonsterSetBaseCustomTwo();
	virtual ~CMonsterSetBaseCustomTwo();
	void Load(char* path);
	void SetInfo(MONSTER_SET_BASE_INFO_CUSTOM2 info);
	...
	void SetMonsterDataCustomTwo();
	bool gObjSetPosMonster(int aIndex, int number);
public:
	MONSTER_SET_BASE_INFO_CUSTOM2 m_MonsterSetBaseInfo[MAX_MSB_MONSTER];
	MONSTER_SET_BASE_INFO_CUSTOM2 m_Mp[OBJ_MAXMONSTER];//MC
	int m_count;
};

extern CMonsterSetBaseCustomTwo gMonsterSetBaseCustomTwo;
