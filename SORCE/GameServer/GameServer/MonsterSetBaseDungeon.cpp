// MonsterSetBase.cpp: implementation of the CMonsterSetBaseDungeon class.
//
//////////////////////////////////////////////////////////////////////
...
CMonsterSetBaseDungeon gMonsterSetBaseDungeon;
//////////////////////////////////////////////////////////////////////
// Construction/Destruction
//////////////////////////////////////////////////////////////////////

CMonsterSetBaseDungeon::CMonsterSetBaseDungeon() // OK
{
	this->m_count = 0;
}
...
void CMonsterSetBaseDungeon::Load(char* path) // OK
{
	...
	try
	{
		while (true)
		{
			...
			while (true)
			{
				...
				MONSTER_SET_BASE_INFO_DUNGEON info;
...
void CMonsterSetBaseDungeon::SetInfo(MONSTER_SET_BASE_INFO_DUNGEON info) // OK
{
...
bool CMonsterSetBaseDungeon::GetPosition(int index, short map, short* ox, short* oy) // OK
{
	...
	MONSTER_SET_BASE_INFO_DUNGEON* lpInfo = &this->m_MonsterSetBaseInfo[index];
...
bool CMonsterSetBaseDungeon::GetBoxPosition(int map, int x, int y, int tx, int ty, short* ox, short* oy) // OK
{
	for (int n = 0; n < 100; n++)
	{
		int subx = tx - x;
...
void CMonsterSetBaseDungeon::SetBoxPosition(int index, int map, int x, int y, int tx, int ty) // OK
{
	if (index < 0 || index >= MAX_MSB_MONSTER)
	{
		return;
...
void CMonsterSetBaseDungeon::SetMonsterDataDungeon() // OK
{
	for (int n = 0; n < gMonsterSetBaseDungeon.m_count; n++)
	{
		MONSTER_SET_BASE_INFO_DUNGEON* lpInfo = &gMonsterSetBaseDungeon.m_MonsterSetBaseInfo[n];
...
bool CMonsterSetBaseDungeon::gObjSetPosMonster(int aIndex, int number) // OK
{
	...
	MONSTER_SET_BASE_INFO_DUNGEON* lpInfo = &gMonsterSetBaseDungeon.m_MonsterSetBaseInfo[number];

	short x, y;

	if (gMonsterSetBaseDungeon.GetPosition(number, lpInfo->Map, &x, &y) == 0)
	{
		return 0;
	}
