...
struct MONSTER_SET_BASE_INFO_DUNGEON
{
	int Type;
	int MonsterClass;
	int Map;
...
class CMonsterSetBaseDungeon
{
public:
	CMonsterSetBaseDungeon();
	virtual ~CMonsterSetBaseDungeon();
	void Load(char* path);
	void SetInfo(MONSTER_SET_BASE_INFO_DUNGEON info);
	...
	void SetMonsterDataDungeon();
	bool gObjSetPosMonster(int aIndex, int number);
public:
	MONSTER_SET_BASE_INFO_DUNGEON m_MonsterSetBaseInfo[MAX_MSB_MONSTER];
	MONSTER_SET_BASE_INFO_DUNGEON m_Mp[OBJ_MAXMONSTER];//MC
	int m_count;
};

extern CMonsterSetBaseDungeon gMonsterSetBaseDungeon;
