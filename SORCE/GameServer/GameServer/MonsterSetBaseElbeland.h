...
struct MONSTER_SET_BASE_INFO_ELBELAND
{
	int Type;
	int MonsterClass;
	int Map;
...
class CMonsterSetBaseElbeland
{
public:
	CMonsterSetBaseElbeland();
	virtual ~CMonsterSetBaseElbeland();
	void Load(char* path);
	void SetInfo(MONSTER_SET_BASE_INFO_ELBELAND info);
	...
	void SetMonsterDataElbeland();
	bool gObjSetPosMonster(int aIndex, int number);
public:
	MONSTER_SET_BASE_INFO_ELBELAND m_MonsterSetBaseInfo[MAX_MSB_MONSTER];
	MONSTER_SET_BASE_INFO_ELBELAND m_Mp[OBJ_MAXMONSTER];//MC
	int m_count;
};

extern CMonsterSetBaseElbeland gMonsterSetBaseElbeland;
