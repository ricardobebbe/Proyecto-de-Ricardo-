// MonsterSetBase.cpp: implementation of the CMonsterSetBaseIcarus class.
//
//////////////////////////////////////////////////////////////////////
...
CMonsterSetBaseIcarus gMonsterSetBaseIcarus;
//////////////////////////////////////////////////////////////////////
// Construction/Destruction
//////////////////////////////////////////////////////////////////////

CMonsterSetBaseIcarus::CMonsterSetBaseIcarus() // OK
{
	this->m_count = 0;
}
...
void CMonsterSetBaseIcarus::Load(char* path) // OK
{
	...
	try
	{
		while (true)
		{
			...
			while (true)
			{
				...
				MONSTER_SET_BASE_INFO_ICARUS info;
...
void CMonsterSetBaseIcarus::SetInfo(MONSTER_SET_BASE_INFO_ICARUS info) // OK
{
...
bool CMonsterSetBaseIcarus::GetPosition(int index, short map, short* ox, short* oy) // OK
{
	...
	MONSTER_SET_BASE_INFO_ICARUS* lpInfo = &this->m_MonsterSetBaseInfo[index];
...
bool CMonsterSetBaseIcarus::GetBoxPosition(int map, int x, int y, int tx, int ty, short* ox, short* oy) // OK
{
	for (int n = 0; n < 100; n++)
	{
		int subx = tx - x;
...
void CMonsterSetBaseIcarus::SetBoxPosition(int index, int map, int x, int y, int tx, int ty) // OK
{
	if (index < 0 || index >= MAX_MSB_MONSTER)
	{
		return;
...
void CMonsterSetBaseIcarus::SetMonsterDataIcarus() // OK
{
	for (int n = 0; n < gMonsterSetBaseIcarus.m_count; n++)
	{
		MONSTER_SET_BASE_INFO_ICARUS* lpInfo = &gMonsterSetBaseIcarus.m_MonsterSetBaseInfo[n];
...
bool CMonsterSetBaseIcarus::gObjSetPosMonster(int aIndex, int number) // OK
{
	...
	MONSTER_SET_BASE_INFO_ICARUS* lpInfo = &gMonsterSetBaseIcarus.m_MonsterSetBaseInfo[number];

	short x, y;

	if (gMonsterSetBaseIcarus.GetPosition(number, lpInfo->Map, &x, &y) == 0)
	{
		return 0;
	}
