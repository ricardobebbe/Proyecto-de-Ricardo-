// MonsterSetBase.cpp: implementation of the CMonsterSetBaseIllusionFiv class.
//
//////////////////////////////////////////////////////////////////////
...
CMonsterSetBaseIllusionFiv gMonsterSetBaseIllusionFiv;
//////////////////////////////////////////////////////////////////////
// Construction/Destruction
//////////////////////////////////////////////////////////////////////

CMonsterSetBaseIllusionFiv::CMonsterSetBaseIllusionFiv() // OK
{
	this->m_count = 0;
}
...
void CMonsterSetBaseIllusionFiv::Load(char* path) // OK
{
	...
	try
	{
		while (true)
		{
			...
			while (true)
			{
				...
				MONSTER_SET_BASE_INFO_ILLUSIONTEMPLE5 info;
...
void CMonsterSetBaseIllusionFiv::SetInfo(MONSTER_SET_BASE_INFO_ILLUSIONTEMPLE5 info) // OK
{
...
bool CMonsterSetBaseIllusionFiv::GetPosition(int index, short map, short* ox, short* oy) // OK
{
	...
	MONSTER_SET_BASE_INFO_ILLUSIONTEMPLE5* lpInfo = &this->m_MonsterSetBaseInfo[index];
...
bool CMonsterSetBaseIllusionFiv::GetBoxPosition(int map, int x, int y, int tx, int ty, short* ox, short* oy) // OK
{
	for (int n = 0; n < 100; n++)
	{
		int subx = tx - x;
...
void CMonsterSetBaseIllusionFiv::SetBoxPosition(int index, int map, int x, int y, int tx, int ty) // OK
{
	if (index < 0 || index >= MAX_MSB_MONSTER)
	{
		return;
...
void CMonsterSetBaseIllusionFiv::SetMonsterDataIllusionFiv() // OK
{
	for (int n = 0; n < gMonsterSetBaseIllusionFiv.m_count; n++)
	{
		MONSTER_SET_BASE_INFO_ILLUSIONTEMPLE5* lpInfo = &gMonsterSetBaseIllusionFiv.m_MonsterSetBaseInfo[n];
...
bool CMonsterSetBaseIllusionFiv::gObjSetPosMonster(int aIndex, int number) // OK
{
	...
	MONSTER_SET_BASE_INFO_ILLUSIONTEMPLE5* lpInfo = &gMonsterSetBaseIllusionFiv.m_MonsterSetBaseInfo[number];

	short x, y;

	if (gMonsterSetBaseIllusionFiv.GetPosition(number, lpInfo->Map, &x, &y) == 0)
	{
		return 0;
	}
