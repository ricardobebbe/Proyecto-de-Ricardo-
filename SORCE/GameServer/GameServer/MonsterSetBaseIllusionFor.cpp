// MonsterSetBase.cpp: implementation of the CMonsterSetBaseIllusionFor class.
//
//////////////////////////////////////////////////////////////////////
...
CMonsterSetBaseIllusionFor gMonsterSetBaseIllusionFor;
//////////////////////////////////////////////////////////////////////
// Construction/Destruction
//////////////////////////////////////////////////////////////////////

CMonsterSetBaseIllusionFor::CMonsterSetBaseIllusionFor() // OK
{
	this->m_count = 0;
}
...
void CMonsterSetBaseIllusionFor::Load(char* path) // OK
{
	...
	try
	{
		while (true)
		{
			...
			while (true)
			{
				...
				MONSTER_SET_BASE_INFO_ILLUSIONTEMPLE4 info;
...
void CMonsterSetBaseIllusionFor::SetInfo(MONSTER_SET_BASE_INFO_ILLUSIONTEMPLE4 info) // OK
{
...
bool CMonsterSetBaseIllusionFor::GetPosition(int index, short map, short* ox, short* oy) // OK
{
	...
	MONSTER_SET_BASE_INFO_ILLUSIONTEMPLE4* lpInfo = &this->m_MonsterSetBaseInfo[index];
...
bool CMonsterSetBaseIllusionFor::GetBoxPosition(int map, int x, int y, int tx, int ty, short* ox, short* oy) // OK
{
	for (int n = 0; n < 100; n++)
	{
		int subx = tx - x;
...
void CMonsterSetBaseIllusionFor::SetBoxPosition(int index, int map, int x, int y, int tx, int ty) // OK
{
	if (index < 0 || index >= MAX_MSB_MONSTER)
	{
		return;
...
void CMonsterSetBaseIllusionFor::SetMonsterDataIllusionFor() // OK
{
	for (int n = 0; n < gMonsterSetBaseIllusionFor.m_count; n++)
	{
		MONSTER_SET_BASE_INFO_ILLUSIONTEMPLE4* lpInfo = &gMonsterSetBaseIllusionFor.m_MonsterSetBaseInfo[n];
...
bool CMonsterSetBaseIllusionFor::gObjSetPosMonster(int aIndex, int number) // OK
{
	...
	MONSTER_SET_BASE_INFO_ILLUSIONTEMPLE4* lpInfo = &gMonsterSetBaseIllusionFor.m_MonsterSetBaseInfo[number];

	short x, y;

	if (gMonsterSetBaseIllusionFor.GetPosition(number, lpInfo->Map, &x, &y) == 0)
	{
		return 0;
	}
