// MonsterSetBase.cpp: implementation of the CMonsterSetBaseIllusionSix class.
//
//////////////////////////////////////////////////////////////////////
...
CMonsterSetBaseIllusionSix gMonsterSetBaseIllusionSix;
//////////////////////////////////////////////////////////////////////
// Construction/Destruction
//////////////////////////////////////////////////////////////////////

CMonsterSetBaseIllusionSix::CMonsterSetBaseIllusionSix() // OK
{
	this->m_count = 0;
}
...
void CMonsterSetBaseIllusionSix::Load(char* path) // OK
{
	...
	try
	{
		while (true)
		{
			...
			while (true)
			{
				...
				MONSTER_SET_BASE_INFO_ILLUSIONTEMPLE6 info;
...
void CMonsterSetBaseIllusionSix::SetInfo(MONSTER_SET_BASE_INFO_ILLUSIONTEMPLE6 info) // OK
{
...
bool CMonsterSetBaseIllusionSix::GetPosition(int index, short map, short* ox, short* oy) // OK
{
	...
	MONSTER_SET_BASE_INFO_ILLUSIONTEMPLE6* lpInfo = &this->m_MonsterSetBaseInfo[index];
...
bool CMonsterSetBaseIllusionSix::GetBoxPosition(int map, int x, int y, int tx, int ty, short* ox, short* oy) // OK
{
	for (int n = 0; n < 100; n++)
	{
		int subx = tx - x;
...
void CMonsterSetBaseIllusionSix::SetBoxPosition(int index, int map, int x, int y, int tx, int ty) // OK
{
	if (index < 0 || index >= MAX_MSB_MONSTER)
	{
		return;
...
void CMonsterSetBaseIllusionSix::SetMonsterDataIllusionSix() // OK
{
	for (int n = 0; n < gMonsterSetBaseIllusionSix.m_count; n++)
	{
		MONSTER_SET_BASE_INFO_ILLUSIONTEMPLE6* lpInfo = &gMonsterSetBaseIllusionSix.m_MonsterSetBaseInfo[n];
...
bool CMonsterSetBaseIllusionSix::gObjSetPosMonster(int aIndex, int number) // OK
{
	...
	MONSTER_SET_BASE_INFO_ILLUSIONTEMPLE6* lpInfo = &gMonsterSetBaseIllusionSix.m_MonsterSetBaseInfo[number];

	short x, y;

	if (gMonsterSetBaseIllusionSix.GetPosition(number, lpInfo->Map, &x, &y) == 0)
	{
		return 0;
	}
