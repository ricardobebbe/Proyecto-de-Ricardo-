// MonsterSetBase.cpp: implementation of the CMonsterSetBaseIllusionThe class.
//
//////////////////////////////////////////////////////////////////////
...
CMonsterSetBaseIllusionThe gMonsterSetBaseIllusionThe;
//////////////////////////////////////////////////////////////////////
// Construction/Destruction
//////////////////////////////////////////////////////////////////////

CMonsterSetBaseIllusionThe::CMonsterSetBaseIllusionThe() // OK
{
	this->m_count = 0;
}
...
void CMonsterSetBaseIllusionThe::Load(char* path) // OK
{
	...
	try
	{
		while (true)
		{
			...
			while (true)
			{
				...
				MONSTER_SET_BASE_INFO_ILLUSIONTEMPLE3 info;
...
void CMonsterSetBaseIllusionThe::SetInfo(MONSTER_SET_BASE_INFO_ILLUSIONTEMPLE3 info) // OK
{
...
bool CMonsterSetBaseIllusionThe::GetPosition(int index, short map, short* ox, short* oy) // OK
{
	...
	MONSTER_SET_BASE_INFO_ILLUSIONTEMPLE3* lpInfo = &this->m_MonsterSetBaseInfo[index];
...
bool CMonsterSetBaseIllusionThe::GetBoxPosition(int map, int x, int y, int tx, int ty, short* ox, short* oy) // OK
{
	for (int n = 0; n < 100; n++)
	{
		int subx = tx - x;
...
void CMonsterSetBaseIllusionThe::SetBoxPosition(int index, int map, int x, int y, int tx, int ty) // OK
{
	if (index < 0 || index >= MAX_MSB_MONSTER)
	{
		return;
...
void CMonsterSetBaseIllusionThe::SetMonsterDataIllusionThe() // OK
{
	for (int n = 0; n < gMonsterSetBaseIllusionThe.m_count; n++)
	{
		MONSTER_SET_BASE_INFO_ILLUSIONTEMPLE3* lpInfo = &gMonsterSetBaseIllusionThe.m_MonsterSetBaseInfo[n];
...
bool CMonsterSetBaseIllusionThe::gObjSetPosMonster(int aIndex, int number) // OK
{
	...
	MONSTER_SET_BASE_INFO_ILLUSIONTEMPLE3* lpInfo = &gMonsterSetBaseIllusionThe.m_MonsterSetBaseInfo[number];

	short x, y;

	if (gMonsterSetBaseIllusionThe.GetPosition(number, lpInfo->Map, &x, &y) == 0)
	{
		return 0;
	}
