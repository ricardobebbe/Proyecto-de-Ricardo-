// MonsterSetBase.cpp: implementation of the CMonsterSetBaseImperialFor class.
//
//////////////////////////////////////////////////////////////////////
...
CMonsterSetBaseImperialFor gMonsterSetBaseImperialFor;
//////////////////////////////////////////////////////////////////////
// Construction/Destruction
//////////////////////////////////////////////////////////////////////

CMonsterSetBaseImperialFor::CMonsterSetBaseImperialFor() // OK
{
	this->m_count = 0;
}
...
void CMonsterSetBaseImperialFor::Load(char* path) // OK
{
	...
	try
	{
		while (true)
		{
			...
			while (true)
			{
				...
				MONSTER_SET_BASE_INFO_IMPERIAL4 info;
...
void CMonsterSetBaseImperialFor::SetInfo(MONSTER_SET_BASE_INFO_IMPERIAL4 info) // OK
{
...
bool CMonsterSetBaseImperialFor::GetPosition(int index, short map, short* ox, short* oy) // OK
{
	...
	MONSTER_SET_BASE_INFO_IMPERIAL4* lpInfo = &this->m_MonsterSetBaseInfo[index];
...
bool CMonsterSetBaseImperialFor::GetBoxPosition(int map, int x, int y, int tx, int ty, short* ox, short* oy) // OK
{
	for (int n = 0; n < 100; n++)
	{
		int subx = tx - x;
...
void CMonsterSetBaseImperialFor::SetBoxPosition(int index, int map, int x, int y, int tx, int ty) // OK
{
	if (index < 0 || index >= MAX_MSB_MONSTER)
	{
		return;
...
void CMonsterSetBaseImperialFor::SetMonsterDataImperialFor() // OK
{
	for (int n = 0; n < gMonsterSetBaseImperialFor.m_count; n++)
	{
		MONSTER_SET_BASE_INFO_IMPERIAL4* lpInfo = &gMonsterSetBaseImperialFor.m_MonsterSetBaseInfo[n];
...
bool CMonsterSetBaseImperialFor::gObjSetPosMonster(int aIndex, int number) // OK
{
	...
	MONSTER_SET_BASE_INFO_IMPERIAL4* lpInfo = &gMonsterSetBaseImperialFor.m_MonsterSetBaseInfo[number];

	short x, y;

	if (gMonsterSetBaseImperialFor.GetPosition(number, lpInfo->Map, &x, &y) == 0)
	{
		return 0;
	}
