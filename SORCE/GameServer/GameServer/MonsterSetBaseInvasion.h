...
struct MONSTER_SET_BASE_INFO_INVASION
{
	int Type;
	int MonsterClass;
	int Map;
...
class CMonsterSetBaseInvasion
{
public:
	CMonsterSetBaseInvasion();
	virtual ~CMonsterSetBaseInvasion();
	void Load(char* path);
	void SetInfo(MONSTER_SET_BASE_INFO_INVASION info);
	...
	void SetMonsterDataInvasion();
	bool gObjSetPosMonster(int aIndex, int number);
public:
	MONSTER_SET_BASE_INFO_INVASION m_MonsterSetBaseInfo[MAX_MSB_MONSTER];
	MONSTER_SET_BASE_INFO_INVASION m_Mp[OBJ_MAXMONSTER];//MC
	int m_count;
};

extern CMonsterSetBaseInvasion gMonsterSetBaseInvasion;
