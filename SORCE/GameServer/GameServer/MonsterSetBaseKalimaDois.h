...
struct MONSTER_SET_BASE_INFO_KALIMA2
{
	int Type;
	int MonsterClass;
	int Map;
...
class CMonsterSetBaseKalimaTwo
{
public:
	CMonsterSetBaseKalimaTwo();
	virtual ~CMonsterSetBaseKalimaTwo();
	void Load(char* path);
	void SetInfo(MONSTER_SET_BASE_INFO_KALIMA2 info);
	...
	void SetMonsterDataKalimaTwo();
	bool gObjSetPosMonster(int aIndex, int number);
public:
	MONSTER_SET_BASE_INFO_KALIMA2 m_MonsterSetBaseInfo[MAX_MSB_MONSTER];
	MONSTER_SET_BASE_INFO_KALIMA2 m_Mp[OBJ_MAXMONSTER];//MC
	int m_count;
};

extern CMonsterSetBaseKalimaTwo gMonsterSetBaseKalimaTwo;
