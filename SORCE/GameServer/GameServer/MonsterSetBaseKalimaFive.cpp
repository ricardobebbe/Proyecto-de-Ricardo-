// MonsterSetBase.cpp: implementation of the CMonsterSetBaseKalimaFive class.
//
//////////////////////////////////////////////////////////////////////
...
CMonsterSetBaseKalimaFive gMonsterSetBaseKalimaFive;
//////////////////////////////////////////////////////////////////////
// Construction/Destruction
//////////////////////////////////////////////////////////////////////

CMonsterSetBaseKalimaFive::CMonsterSetBaseKalimaFive() // OK
{
	this->m_count = 0;
}
...
void CMonsterSetBaseKalimaFive::Load(char* path) // OK
{
	...
	try
	{
		while (true)
		{
			...
			while (true)
			{
				...
				MONSTER_SET_BASE_INFO_KALIMA5 info;
...
void CMonsterSetBaseKalimaFive::SetInfo(MONSTER_SET_BASE_INFO_KALIMA5 info) // OK
{
...
bool CMonsterSetBaseKalimaFive::GetPosition(int index, short map, short* ox, short* oy) // OK
{
	...
	MONSTER_SET_BASE_INFO_KALIMA5* lpInfo = &this->m_MonsterSetBaseInfo[index];
...
bool CMonsterSetBaseKalimaFive::GetBoxPosition(int map, int x, int y, int tx, int ty, short* ox, short* oy) // OK
{
	for (int n = 0; n < 100; n++)
	{
		int subx = tx - x;
...
void CMonsterSetBaseKalimaFive::SetBoxPosition(int index, int map, int x, int y, int tx, int ty) // OK
{
	if (index < 0 || index >= MAX_MSB_MONSTER)
	{
		return;
...
void CMonsterSetBaseKalimaFive::SetMonsterDataKalimaFive() // OK
{
	for (int n = 0; n < gMonsterSetBaseKalimaFive.m_count; n++)
	{
		MONSTER_SET_BASE_INFO_KALIMA5* lpInfo = &gMonsterSetBaseKalimaFive.m_MonsterSetBaseInfo[n];
...
bool CMonsterSetBaseKalimaFive::gObjSetPosMonster(int aIndex, int number) // OK
{
	...
	MONSTER_SET_BASE_INFO_KALIMA5* lpInfo = &gMonsterSetBaseKalimaFive.m_MonsterSetBaseInfo[number];

	short x, y;

	if (gMonsterSetBaseKalimaFive.GetPosition(number, lpInfo->Map, &x, &y) == 0)
	{
		return 0;
	}
