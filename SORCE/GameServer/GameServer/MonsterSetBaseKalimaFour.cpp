// MonsterSetBase.cpp: implementation of the CMonsterSetBaseKalimaFour class.
//
//////////////////////////////////////////////////////////////////////
...
CMonsterSetBaseKalimaFour gMonsterSetBaseKalimaFour;
//////////////////////////////////////////////////////////////////////
// Construction/Destruction
//////////////////////////////////////////////////////////////////////

CMonsterSetBaseKalimaFour::CMonsterSetBaseKalimaFour() // OK
{
	this->m_count = 0;
}
...
void CMonsterSetBaseKalimaFour::Load(char* path) // OK
{
	...
	try
	{
		while (true)
		{
			...
			while (true)
			{
				...
				MONSTER_SET_BASE_INFO_KALIMA4 info;
...
void CMonsterSetBaseKalimaFour::SetInfo(MONSTER_SET_BASE_INFO_KALIMA4 info) // OK
{
...
bool CMonsterSetBaseKalimaFour::GetPosition(int index, short map, short* ox, short* oy) // OK
{
	...
	MONSTER_SET_BASE_INFO_KALIMA4* lpInfo = &this->m_MonsterSetBaseInfo[index];
...
bool CMonsterSetBaseKalimaFour::GetBoxPosition(int map, int x, int y, int tx, int ty, short* ox, short* oy) // OK
{
	for (int n = 0; n < 100; n++)
	{
		int subx = tx - x;
...
void CMonsterSetBaseKalimaFour::SetBoxPosition(int index, int map, int x, int y, int tx, int ty) // OK
{
	if (index < 0 || index >= MAX_MSB_MONSTER)
	{
		return;
...
void CMonsterSetBaseKalimaFour::SetMonsterDataKalimaFour() // OK
{
	for (int n = 0; n < gMonsterSetBaseKalimaFour.m_count; n++)
	{
		MONSTER_SET_BASE_INFO_KALIMA4* lpInfo = &gMonsterSetBaseKalimaFour.m_MonsterSetBaseInfo[n];
...
bool CMonsterSetBaseKalimaFour::gObjSetPosMonster(int aIndex, int number) // OK
{
	...
	MONSTER_SET_BASE_INFO_KALIMA4* lpInfo = &gMonsterSetBaseKalimaFour.m_MonsterSetBaseInfo[number];

	short x, y;

	if (gMonsterSetBaseKalimaFour.GetPosition(number, lpInfo->Map, &x, &y) == 0)
	{
		return 0;
	}
