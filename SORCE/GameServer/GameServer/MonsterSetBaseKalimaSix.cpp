// MonsterSetBase.cpp: implementation of the CMonsterSetBaseKalimaSix class.
//
//////////////////////////////////////////////////////////////////////
...
CMonsterSetBaseKalimaSix gMonsterSetBaseKalimaSix;
//////////////////////////////////////////////////////////////////////
// Construction/Destruction
//////////////////////////////////////////////////////////////////////

CMonsterSetBaseKalimaSix::CMonsterSetBaseKalimaSix() // OK
{
	this->m_count = 0;
}
...
void CMonsterSetBaseKalimaSix::Load(char* path) // OK
{
	...
	try
	{
		while (true)
		{
			...
			while (true)
			{
				...
				MONSTER_SET_BASE_INFO_KALIMA6 info;
...
void CMonsterSetBaseKalimaSix::SetInfo(MONSTER_SET_BASE_INFO_KALIMA6 info) // OK
{
...
bool CMonsterSetBaseKalimaSix::GetPosition(int index, short map, short* ox, short* oy) // OK
{
	...
	MONSTER_SET_BASE_INFO_KALIMA6* lpInfo = &this->m_MonsterSetBaseInfo[index];
...
bool CMonsterSetBaseKalimaSix::GetBoxPosition(int map, int x, int y, int tx, int ty, short* ox, short* oy) // OK
{
	for (int n = 0; n < 100; n++)
	{
		int subx = tx - x;
...
void CMonsterSetBaseKalimaSix::SetBoxPosition(int index, int map, int x, int y, int tx, int ty) // OK
{
	if (index < 0 || index >= MAX_MSB_MONSTER)
	{
		return;
...
void CMonsterSetBaseKalimaSix::SetMonsterDataKalimaSix() // OK
{
	for (int n = 0; n < gMonsterSetBaseKalimaSix.m_count; n++)
	{
		MONSTER_SET_BASE_INFO_KALIMA6* lpInfo = &gMonsterSetBaseKalimaSix.m_MonsterSetBaseInfo[n];
...
bool CMonsterSetBaseKalimaSix::gObjSetPosMonster(int aIndex, int number) // OK
{
	...
	MONSTER_SET_BASE_INFO_KALIMA6* lpInfo = &gMonsterSetBaseKalimaSix.m_MonsterSetBaseInfo[number];

	short x, y;

	if (gMonsterSetBaseKalimaSix.GetPosition(number, lpInfo->Map, &x, &y) == 0)
	{
		return 0;
	}
