...
struct MONSTER_SET_BASE_INFO_KALIMA6
{
	int Type;
	int MonsterClass;
	int Map;
...
class CMonsterSetBaseKalimaSix
{
public:
	CMonsterSetBaseKalimaSix();
	virtual ~CMonsterSetBaseKalimaSix();
	void Load(char* path);
	void SetInfo(MONSTER_SET_BASE_INFO_KALIMA6 info);
	...
	void SetMonsterDataKalimaSix();
	bool gObjSetPosMonster(int aIndex, int number);
public:
	MONSTER_SET_BASE_INFO_KALIMA6 m_MonsterSetBaseInfo[MAX_MSB_MONSTER];
	MONSTER_SET_BASE_INFO_KALIMA6 m_Mp[OBJ_MAXMONSTER];//MC
	int m_count;
};

extern CMonsterSetBaseKalimaSix gMonsterSetBaseKalimaSix;
