// MonsterSetBase.cpp: implementation of the CMonsterSetBaseKalimaOne class.
//
//////////////////////////////////////////////////////////////////////
...
CMonsterSetBaseKalimaOne gMonsterSetBaseKalimaOne;
//////////////////////////////////////////////////////////////////////
// Construction/Destruction
//////////////////////////////////////////////////////////////////////

CMonsterSetBaseKalimaOne::CMonsterSetBaseKalimaOne() // OK
{
	this->m_count = 0;
}
...
void CMonsterSetBaseKalimaOne::Load(char* path) // OK
{
	...
	try
	{
		while (true)
		{
			...
			while (true)
			{
				...
				MONSTER_SET_BASE_INFO_KALIMA1 info;
...
void CMonsterSetBaseKalimaOne::SetInfo(MONSTER_SET_BASE_INFO_KALIMA1 info) // OK
{
...
bool CMonsterSetBaseKalimaOne::GetPosition(int index, short map, short* ox, short* oy) // OK
{
	...
	MONSTER_SET_BASE_INFO_KALIMA1* lpInfo = &this->m_MonsterSetBaseInfo[index];
...
bool CMonsterSetBaseKalimaOne::GetBoxPosition(int map, int x, int y, int tx, int ty, short* ox, short* oy) // OK
{
	for (int n = 0; n < 100; n++)
	{
		int subx = tx - x;
...
void CMonsterSetBaseKalimaOne::SetBoxPosition(int index, int map, int x, int y, int tx, int ty) // OK
{
	if (index < 0 || index >= MAX_MSB_MONSTER)
	{
		return;
...
void CMonsterSetBaseKalimaOne::SetMonsterDataKalimaOne() // OK
{
	for (int n = 0; n < gMonsterSetBaseKalimaOne.m_count; n++)
	{
		MONSTER_SET_BASE_INFO_KALIMA1* lpInfo = &gMonsterSetBaseKalimaOne.m_MonsterSetBaseInfo[n];
...
bool CMonsterSetBaseKalimaOne::gObjSetPosMonster(int aIndex, int number) // OK
{
	...
	MONSTER_SET_BASE_INFO_KALIMA1* lpInfo = &gMonsterSetBaseKalimaOne.m_MonsterSetBaseInfo[number];

	short x, y;

	if (gMonsterSetBaseKalimaOne.GetPosition(number, lpInfo->Map, &x, &y) == 0)
	{
		return 0;
	}
