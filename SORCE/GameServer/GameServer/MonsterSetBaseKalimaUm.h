...
struct MONSTER_SET_BASE_INFO_KALIMA1
{
	int Type;
	int MonsterClass;
	int Map;
...
class CMonsterSetBaseKalimaOne
{
public:
	CMonsterSetBaseKalimaOne();
	virtual ~CMonsterSetBaseKalimaOne();
	void Load(char* path);
	void SetInfo(MONSTER_SET_BASE_INFO_KALIMA1 info);
	...
	void SetMonsterDataKalimaOne();
	bool gObjSetPosMonster(int aIndex, int number);
public:
	MONSTER_SET_BASE_INFO_KALIMA1 m_MonsterSetBaseInfo[MAX_MSB_MONSTER];
	MONSTER_SET_BASE_INFO_KALIMA1 m_Mp[OBJ_MAXMONSTER];//MC
	int m_count;
};

extern CMonsterSetBaseKalimaOne gMonsterSetBaseKalimaOne;
