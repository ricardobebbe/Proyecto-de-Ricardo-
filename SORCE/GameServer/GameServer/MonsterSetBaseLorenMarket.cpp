// MonsterSetBase.cpp: implementation of the CMonsterSetBaseLorenMarket class.
//
//////////////////////////////////////////////////////////////////////
...
CMonsterSetBaseLorenMarket gMonsterSetBaseLorenMarket;
//////////////////////////////////////////////////////////////////////
// Construction/Destruction
//////////////////////////////////////////////////////////////////////

CMonsterSetBaseLorenMarket::CMonsterSetBaseLorenMarket() // OK
{
	this->m_count = 0;
}
...
void CMonsterSetBaseLorenMarket::Load(char* path) // OK
{
	...
	try
	{
		while (true)
		{
			...
			while (true)
			{
				...
				MONSTER_SET_BASE_INFO_LORENMARKET info;
...
void CMonsterSetBaseLorenMarket::SetInfo(MONSTER_SET_BASE_INFO_LORENMARKET info) // OK
{
...
bool CMonsterSetBaseLorenMarket::GetPosition(int index, short map, short* ox, short* oy) // OK
{
	...
	MONSTER_SET_BASE_INFO_LORENMARKET* lpInfo = &this->m_MonsterSetBaseInfo[index];
...
bool CMonsterSetBaseLorenMarket::GetBoxPosition(int map, int x, int y, int tx, int ty, short* ox, short* oy) // OK
{
	for (int n = 0; n < 100; n++)
	{
		int subx = tx - x;
...
void CMonsterSetBaseLorenMarket::SetBoxPosition(int index, int map, int x, int y, int tx, int ty) // OK
{
	if (index < 0 || index >= MAX_MSB_MONSTER)
	{
		return;
...
void CMonsterSetBaseLorenMarket::SetMonsterDataLorenMarket() // OK
{
	for (int n = 0; n < gMonsterSetBaseLorenMarket.m_count; n++)
	{
		MONSTER_SET_BASE_INFO_LORENMARKET* lpInfo = &gMonsterSetBaseLorenMarket.m_MonsterSetBaseInfo[n];
...
bool CMonsterSetBaseLorenMarket::gObjSetPosMonster(int aIndex, int number) // OK
{
	...
	MONSTER_SET_BASE_INFO_LORENMARKET* lpInfo = &gMonsterSetBaseLorenMarket.m_MonsterSetBaseInfo[number];

	short x, y;

	if (gMonsterSetBaseLorenMarket.GetPosition(number, lpInfo->Map, &x, &y) == 0)
	{
		return 0;
	}
