// MonsterSetBase.cpp: implementation of the CMonsterSetBaseRaklion class.
//
//////////////////////////////////////////////////////////////////////
...
CMonsterSetBaseRaklion gMonsterSetBaseRaklion;
//////////////////////////////////////////////////////////////////////
// Construction/Destruction
//////////////////////////////////////////////////////////////////////

CMonsterSetBaseRaklion::CMonsterSetBaseRaklion() // OK
{
	this->m_count = 0;
}
...
void CMonsterSetBaseRaklion::Load(char* path) // OK
{
	...
	try
	{
		while (true)
		{
			...
			while (true)
			{
				...
				MONSTER_SET_BASE_INFO_RAKLION info;
...
void CMonsterSetBaseRaklion::SetInfo(MONSTER_SET_BASE_INFO_RAKLION info) // OK
{
...
bool CMonsterSetBaseRaklion::GetPosition(int index, short map, short* ox, short* oy) // OK
{
	...
	MONSTER_SET_BASE_INFO_RAKLION* lpInfo = &this->m_MonsterSetBaseInfo[index];
...
bool CMonsterSetBaseRaklion::GetBoxPosition(int map, int x, int y, int tx, int ty, short* ox, short* oy) // OK
{
	for (int n = 0; n < 100; n++)
	{
		int subx = tx - x;
...
void CMonsterSetBaseRaklion::SetBoxPosition(int index, int map, int x, int y, int tx, int ty) // OK
{
	if (index < 0 || index >= MAX_MSB_MONSTER)
	{
		return;
...
void CMonsterSetBaseRaklion::SetMonsterDataRaklion() // OK
{
	for (int n = 0; n < gMonsterSetBaseRaklion.m_count; n++)
	{
		MONSTER_SET_BASE_INFO_RAKLION* lpInfo = &gMonsterSetBaseRaklion.m_MonsterSetBaseInfo[n];
...
bool CMonsterSetBaseRaklion::gObjSetPosMonster(int aIndex, int number) // OK
{
	...
	MONSTER_SET_BASE_INFO_RAKLION* lpInfo = &gMonsterSetBaseRaklion.m_MonsterSetBaseInfo[number];

	short x, y;

	if (gMonsterSetBaseRaklion.GetPosition(number, lpInfo->Map, &x, &y) == 0)
	{
		return 0;
	}
