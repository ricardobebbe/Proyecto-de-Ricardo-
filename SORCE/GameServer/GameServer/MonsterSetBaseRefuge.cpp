// MonsterSetBase.cpp: implementation of the CMonsterSetBaseRefuge class.
//
//////////////////////////////////////////////////////////////////////
...
CMonsterSetBaseRefuge gMonsterSetBaseRefuge;
//////////////////////////////////////////////////////////////////////
// Construction/Destruction
//////////////////////////////////////////////////////////////////////

CMonsterSetBaseRefuge::CMonsterSetBaseRefuge() // OK
{
	this->m_count = 0;
}
...
void CMonsterSetBaseRefuge::Load(char* path) // OK
{
	...
	try
	{
		while (true)
		{
			...
			while (true)
			{
				...
				MONSTER_SET_BASE_INFO_REFUGE info;
...
void CMonsterSetBaseRefuge::SetInfo(MONSTER_SET_BASE_INFO_REFUGE info) // OK
{
...
bool CMonsterSetBaseRefuge::GetPosition(int index, short map, short* ox, short* oy) // OK
{
	...
	MONSTER_SET_BASE_INFO_REFUGE* lpInfo = &this->m_MonsterSetBaseInfo[index];
...
bool CMonsterSetBaseRefuge::GetBoxPosition(int map, int x, int y, int tx, int ty, short* ox, short* oy) // OK
{
	for (int n = 0; n < 100; n++)
	{
		int subx = tx - x;
...
void CMonsterSetBaseRefuge::SetBoxPosition(int index, int map, int x, int y, int tx, int ty) // OK
{
	if (index < 0 || index >= MAX_MSB_MONSTER)
	{
		return;
...
void CMonsterSetBaseRefuge::SetMonsterDataRefuge() // OK
{
	for (int n = 0; n < gMonsterSetBaseRefuge.m_count; n++)
	{
		MONSTER_SET_BASE_INFO_REFUGE* lpInfo = &gMonsterSetBaseRefuge.m_MonsterSetBaseInfo[n];
...
bool CMonsterSetBaseRefuge::gObjSetPosMonster(int aIndex, int number) // OK
{
	...
	MONSTER_SET_BASE_INFO_REFUGE* lpInfo = &gMonsterSetBaseRefuge.m_MonsterSetBaseInfo[number];

	short x, y;

	if (gMonsterSetBaseRefuge.GetPosition(number, lpInfo->Map, &x, &y) == 0)
	{
		return 0;
	}
