// MonsterSetBase.cpp: implementation of the CMonsterSetBaseSantaTown class.
//
//////////////////////////////////////////////////////////////////////
...
CMonsterSetBaseSantaTown gMonsterSetBaseSantaTown;
//////////////////////////////////////////////////////////////////////
// Construction/Destruction
//////////////////////////////////////////////////////////////////////

CMonsterSetBaseSantaTown::CMonsterSetBaseSantaTown() // OK
{
	this->m_count = 0;
}
...
void CMonsterSetBaseSantaTown::Load(char* path) // OK
{
	...
	try
	{
		while (true)
		{
			...
			while (true)
			{
				...
				MONSTER_SET_BASE_INFO_SANTATOWN info;
...
void CMonsterSetBaseSantaTown::SetInfo(MONSTER_SET_BASE_INFO_SANTATOWN info) // OK
{
...
bool CMonsterSetBaseSantaTown::GetPosition(int index, short map, short* ox, short* oy) // OK
{
	...
	MONSTER_SET_BASE_INFO_SANTATOWN* lpInfo = &this->m_MonsterSetBaseInfo[index];
...
bool CMonsterSetBaseSantaTown::GetBoxPosition(int map, int x, int y, int tx, int ty, short* ox, short* oy) // OK
{
	for (int n = 0; n < 100; n++)
	{
		int subx = tx - x;
...
void CMonsterSetBaseSantaTown::SetBoxPosition(int index, int map, int x, int y, int tx, int ty) // OK
{
	if (index < 0 || index >= MAX_MSB_MONSTER)
	{
		return;
...
void CMonsterSetBaseSantaTown::SetMonsterDataSantaTown() // OK
{
	for (int n = 0; n < gMonsterSetBaseSantaTown.m_count; n++)
	{
		MONSTER_SET_BASE_INFO_SANTATOWN* lpInfo = &gMonsterSetBaseSantaTown.m_MonsterSetBaseInfo[n];
...
bool CMonsterSetBaseSantaTown::gObjSetPosMonster(int aIndex, int number) // OK
{
	...
	MONSTER_SET_BASE_INFO_SANTATOWN* lpInfo = &gMonsterSetBaseSantaTown.m_MonsterSetBaseInfo[number];

	short x, y;

	if (gMonsterSetBaseSantaTown.GetPosition(number, lpInfo->Map, &x, &y) == 0)
	{
		return 0;
	}
