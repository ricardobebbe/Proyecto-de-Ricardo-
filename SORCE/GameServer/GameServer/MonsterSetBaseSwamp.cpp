// MonsterSetBase.cpp: implementation of the CMonsterSetBaseSwamp class.
//
//////////////////////////////////////////////////////////////////////
...
CMonsterSetBaseSwamp gMonsterSetBaseSwamp;
//////////////////////////////////////////////////////////////////////
// Construction/Destruction
//////////////////////////////////////////////////////////////////////

CMonsterSetBaseSwamp::CMonsterSetBaseSwamp() // OK
{
	this->m_count = 0;
}
...
void CMonsterSetBaseSwamp::Load(char* path) // OK
{
	...
	try
	{
		while (true)
		{
			...
			while (true)
			{
				...
				MONSTER_SET_BASE_INFO_SWAMP info;
...
void CMonsterSetBaseSwamp::SetInfo(MONSTER_SET_BASE_INFO_SWAMP info) // OK
{
...
bool CMonsterSetBaseSwamp::GetPosition(int index, short map, short* ox, short* oy) // OK
{
	...
	MONSTER_SET_BASE_INFO_SWAMP* lpInfo = &this->m_MonsterSetBaseInfo[index];
...
bool CMonsterSetBaseSwamp::GetBoxPosition(int map, int x, int y, int tx, int ty, short* ox, short* oy) // OK
{
	for (int n = 0; n < 100; n++)
	{
		int subx = tx - x;
...
void CMonsterSetBaseSwamp::SetBoxPosition(int index, int map, int x, int y, int tx, int ty) // OK
{
	if (index < 0 || index >= MAX_MSB_MONSTER)
	{
		return;
...
void CMonsterSetBaseSwamp::SetMonsterDataSwamp() // OK
{
	for (int n = 0; n < gMonsterSetBaseSwamp.m_count; n++)
	{
		MONSTER_SET_BASE_INFO_SWAMP* lpInfo = &gMonsterSetBaseSwamp.m_MonsterSetBaseInfo[n];
...
bool CMonsterSetBaseSwamp::gObjSetPosMonster(int aIndex, int number) // OK
{
	...
	MONSTER_SET_BASE_INFO_SWAMP* lpInfo = &gMonsterSetBaseSwamp.m_MonsterSetBaseInfo[number];

	short x, y;

	if (gMonsterSetBaseSwamp.GetPosition(number, lpInfo->Map, &x, &y) == 0)
	{
		return 0;
	}
