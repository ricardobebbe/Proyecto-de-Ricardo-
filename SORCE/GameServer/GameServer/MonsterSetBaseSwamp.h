...
struct MONSTER_SET_BASE_INFO_SWAMP
{
	int Type;
	int MonsterClass;
	int Map;
...
class CMonsterSetBaseSwamp
{
public:
	CMonsterSetBaseSwamp();
	virtual ~CMonsterSetBaseSwamp();
	void Load(char* path);
	void SetInfo(MONSTER_SET_BASE_INFO_SWAMP info);
	...
	void SetMonsterDataSwamp();
	bool gObjSetPosMonster(int aIndex, int number);
public:
	MONSTER_SET_BASE_INFO_SWAMP m_MonsterSetBaseInfo[MAX_MSB_MONSTER];
	MONSTER_SET_BASE_INFO_SWAMP m_Mp[OBJ_MAXMONSTER];//MC
	int m_count;
};

extern CMonsterSetBaseSwamp gMonsterSetBaseSwamp;
