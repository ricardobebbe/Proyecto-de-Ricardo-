...
struct MONSTER_SET_BASE_INFO_TARKAN
{
	int Type;
	int MonsterClass;
	int Map;
...
class CMonsterSetBaseTarkan
{
public:
	CMonsterSetBaseTarkan();
	virtual ~CMonsterSetBaseTarkan();
	void Load(char* path);
	void SetInfo(MONSTER_SET_BASE_INFO_TARKAN info);
	...
	void SetMonsterDataTarkan();
	bool gObjSetPosMonster(int aIndex, int number);
public:
	MONSTER_SET_BASE_INFO_TARKAN m_MonsterSetBaseInfo[MAX_MSB_MONSTER];
	MONSTER_SET_BASE_INFO_TARKAN m_Mp[OBJ_MAXMONSTER];//MC
	int m_count;
};

extern CMonsterSetBaseTarkan gMonsterSetBaseTarkan;
