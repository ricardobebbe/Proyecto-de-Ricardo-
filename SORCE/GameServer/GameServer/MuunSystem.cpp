...
//	#if(GAMESERVER_UPDATE>=803)
	...
	if(ItemIndex == GET_ITEM(13,198) || ItemIndex == GET_ITEM(13,239) || ItemIndex == GET_ITEM(13,240))
	{
		return 1;
	}
	else
...
//	#if(GAMESERVER_UPDATE>=803)
	...
	if(lpItem->m_Index == GET_ITEM(13,198) || lpItem->m_Index == GET_ITEM(13,239) || lpItem->m_Index == GET_ITEM(13,240))
	{
		return 1;
	}
	else
...
//	#if(GAMESERVER_UPDATE>=803)
	...
	if(MUUN_INVENTORY_WEAR_RANGE(slot) == 0||1) //ZEUS UP18 Pasar Pro Inv MuunPet D Lado
	{
		return 1;
	}
...
//	#if(GAMESERVER_UPDATE>=803)
	...
	if(ItemIndex == GET_ITEM(13,198))
	{
		if(this->IsMuunItem((GET_ITEM(13,198)+(*ItemLevel))) == 0)
		{
			int MuunItemIndex = -1;
		...
		{
			int MuunItemIndex = GET_ITEM(13,198)+(*ItemLevel);
...
//	#if(GAMESERVER_UPDATE>=803)
	...
	if(lpObj->MuunInventory[SourceSlot].m_Index != GET_ITEM(13,198))
	{
		return 0;
	}

	if(lpObj->MuunInventory[TargetSlot].m_Level != 0)
	{
		return 0;
	}
	...
	GDCreateItemSend(lpObj->Index,0xED,0,0,MuunSystemInfo.EvolutionItemIndex,0,(BYTE)lpObj->MuunInventory[TargetSlot].m_Durability,0,0,0,lpObj->Index,0,0,0,0,0,0xFF,0);
	...
	for(int n=0;n < MUUN_INVENTORY_WEAR_SIZE;n++)
	{
		...
		if(this->GetMuunSystemInfo(lpObj->MuunInventory[n].m_Index,&MuunSystemInfo) != 0)
		{
			...
			if(this->GetMuunSystemOptionInfo(MuunSystemInfo.OptionIndex,&MuunSystemOptionInfo) != 0)
			{
				...
				//LogAdd(LOG_RED, "InsertOption %d, %d", MuunSystemOptionInfo.Index, value);
			}
			//LogAdd(LOG_RED, "CalcMuunOption %d %d", lpObj->MuunInventory[n].m_Index);
		}
		//LogAdd(LOG_RED, "CalcMuunNewPetOption [%d] %d %d", lpObj->MuunInventory[n].m_Index, MuunSystemInfo.OptionIndex, flag);
...
void CMuunSystem::CGMuunDropItemRecv(PMSG_MUUN_ITEM_DROP_SEND* lpMsg, int aIndex) // OK
{
	...
	GDCreateItemSend(lpObj->Index,0xEB,0,0,lpItem->m_Index,lpItem->m_Level,(BYTE)lpItem->m_Durability,lpItem->m_Option1,lpItem->m_Option2,lpItem->m_Option3,lpObj->Index,lpItem->m_NewOption,lpItem->m_SetOption,lpItem->m_JewelOfHarmonyOption, lpItem->m_ItemOptionEx, lpItem->m_SocketOption,lpItem->m_SocketOptionBonus,lpItem->m_PeriodicItemTime);
