...
struct MUUN_SYSTEM_CUSTOM_INFO
{
	int Index;
	int IncDamageRate;
	int IncLife;
...
public:
	...
	void CGMuunDropItemRecv(PMSG_MUUN_ITEM_DROP_SEND* lpMsg, int aIndex);
	...
	void BMuunPickUp(int aIndex, int ItemIndex);
	void CalcMuunNewPetOption(LPOBJ lpObj, int OptionIndex, bool flag);
private:
	std::map<int,MUUN_SYSTEM_INFO> m_MuunSystemInfo;
	std::map<int,MUUN_SYSTEM_OPTION_INFO> m_MuunSystemOptionInfo;
	std::map<int, MUUN_SYSTEM_CUSTOM_INFO> m_MuunCustomPetInfo;
};
