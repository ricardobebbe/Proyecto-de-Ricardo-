...
bool CNpcTalk::NpcTalk(LPOBJ lpNpc,LPOBJ lpObj) // OK
{
	...
	switch(lpNpc->Class)
	{
		...
		case 602:
			...
			break;
			case 703:
			this->NpcZeusMercador(lpNpc,lpObj);
			break;
#endif
...
void CNpcTalk::NpcElfBuffer(LPOBJ lpNpc,LPOBJ lpObj)  //ZEUS UP19 QUEST ELF DONWGRAND
{	
	//Buffer Elf one
	if(gEffectManager.CheckEffect(lpObj,EFFECT_ELF_BUFFER) != 0)
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,"Você já é abençoado");
		return;
	}
...
void CNpcTalk::NpcLeoTheHelper(LPOBJ lpNpc,LPOBJ lpObj) // OK
{
	...
	//GCServerCommandSend(lpObj->Index,15,1,0,0);   //ZEUSS UP24 LUA
	#endif
}
...
void CNpcTalk::CGNpcTalkRecv(PMSG_NPC_TALK_RECV* lpMsg,int aIndex) // OK
{
	...
	if(gScriptLoader.OnNpcTalk(bIndex,aIndex) != 0) //ZEUS UP24 LUA
	{
		return;
	}
...
void CNpcTalk::CGNpcTalkCloseRecv(int aIndex) // OK
{
	...
	if (lpObj->Interface.use == 0 || lpObj->Interface.type == INTERFACE_NONE || lpObj->Interface.type == INTERFACE_CHAOS_BOX || lpObj->Interface.type == INTERFACE_TRADE || lpObj->Interface.type == INTERFACE_PARTY)
	{
		return;
	}
...
void CNpcTalk::DGNpcLeoTheHelperRecv(SDHP_NPC_LEO_THE_HELPER_RECV* lpMsg) // OK
{
	...
	/*if(lpMsg->status != 0)
	{
		GCServerCommandSend(lpObj->Index,15,0,0,0);  //ZEUSS UP24 LUA
		return;
	}*/
...
void CNpcTalk::NpcZeusMercador(LPOBJ lpNpc, LPOBJ lpObj) //Zeus UP22 Custom ZeusMercador
{
	gCustomZeusMercador.NpcZeusMercador(lpNpc, lpObj);   
	
	// Dialogo
   GCChatTargetSend(lpObj,lpNpc->Index,"Sou Um Bruxo Mercanti,Vim De Muito Longe Para Trocar Renas!!.");
}