...
void CObjectManager::ObjectSetStateProc() // OK
{
	for(int n=0;n < MAX_OBJECT;n++)
	{
		...
		if(lpObj->Type == OBJECT_MONSTER)
		{
			...
			gScriptLoader.OnUserRespawn(lpObj->Index,lpObj->KillerType);  //ZEUS UP24 LUA
...
void CObjectManager::CharacterGameCloseSet(int aIndex,int type) // OK
{
	...
	gScriptLoader.OnCharacterClose(aIndex);  //ZEUS UP24 LUA

	lpObj->CloseCount = 6;

	//lpObj->CloseType = type;

	//ZEUS UP24 Fix GodMode Hack
...
bool CObjectManager::CharacterUseTalismanOfResurrection(LPOBJ lpObj,int SourceSlot) // OK
{
	...
	GCServerCommandSend(lpObj->Index,60,0,0,0);     //ZEUSS UP24 LUA
...
bool CObjectManager::CharacterUseTalismanOfMobility(LPOBJ lpObj,int SourceSlot) // OK
{
	...
	GCServerCommandSend(lpObj->Index,61,0,0,0);     //ZEUSS UP24 LUA
...
void CObjectManager::CharacterLifeCheck(LPOBJ lpObj,LPOBJ lpTarget,int damage,int DamageType,int flag,int type,int skill,int ShieldDamage) // OK
{
	...
	if(lpTarget->Life <= 0 && lpTarget->Live != 0)
	{
		...
		if(lpTarget->Type == OBJECT_MONSTER)
		{
			...
			gScriptLoader.OnMonsterDie(lpTarget->Index,lpObj->Index);  //ZEUS UP24 LUA
		...
		{
			gScriptLoader.OnUserDie(lpTarget->Index,lpObj->Index);  //ZEUS UP24 LUA
		...
		lpTarget->DeathMap = lpObj->Map;  //ZEUS UP24 LUA
		lpTarget->DeathX = lpObj->X;      //ZEUS UP24 LUA
		lpTarget->DeathY = lpObj->Y;      //ZEUS UP24 LUA
		lpTarget->Live = 0;
