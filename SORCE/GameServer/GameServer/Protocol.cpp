...
//Dragones function para chequear mapa de invasin y todos los personajes
...
void ProtocolCore(BYTE head,BYTE* lpMsg,int size,int aIndex,int encrypt,int serial) // OK
{
	...
	if (gObj[aIndex].Type == OBJECT_USER &&lpMsg[0] == 0xC1 && head == 0xFB)
	{
		...
		case 0x16:
			gLuckyWheel.Start(lpObj);   //ZEUS UP24 Cassino
			break;

		default:
			LogAdd(LOG_RED, "Pacote Unk do Cliente(%d) (%x): %x %x %x (%x) %x %x %x %x %x %x", aIndex, head, lpMsg[0], lpMsg[1], lpMsg[2], lpMsg[3], lpMsg[4], lpMsg[5], lpMsg[6], lpMsg[7], lpMsg[8], lpMsg[9]);
			break;
		}
		return;
	}
	...
	switch(head)
	{
		...
		case 0xBF:
			switch(lpMsg[3])
			{
				...
				case 0x70:
					...
					break;
				default:
					LogAdd(LOG_RED, "[0xBF] Pacote desconhecido BF:%d",lpMsg[3]); 
					break;
			}
			break;
		case 0xC0:
		...
		case 0xFC:
			...
			case 0xFD:
			g_NewsBoard.OpenMain(&gObj[aIndex]); //ZEUS UP19 NEW NOTICIAS
			break;	
		case 0xFE:
			g_NewsBoard.OpenItem(&gObj[aIndex], (NEWS_REQ_NEWS*)lpMsg); //ZEUS UP19 NEW NOTICIAS
			break;
#if(CUSTOM_PERSONAL_SHOP==0)    //ZEUS UP24 Mercado Negro IN-GAME STORE
		case 0xDE:
...
void CGChatRecv(PMSG_CHAT_RECV* lpMsg,int aIndex) // OK
{
	...
	if(lpObj->Permission[11] != 0)  //ZEUS UP24 LUA
	{
		return;
	}
...
void CGLiveClientRecv(PMSG_LIVE_CLIENT_RECV* lpMsg,int aIndex) // OK
{
	...
	{
		gLog.Output(LOG_HACK,"[%s][%s] Hack De latncia Do Client Detectado [%d][%d]",lpObj->Account,lpObj->Name,(GetTickCount()-lpObj->ServerTickCount),(lpMsg->TickCount-lpObj->ClientTickCount));
		GCCloseClientSend(aIndex,0);
		return;  
	}
	...
	{
		gLog.Output(LOG_HACK,"[%s][%s] Client Speed Hack Detectado [%d][%d]",lpObj->Account,lpObj->Name,lpObj->PhysiSpeed,lpMsg->PhysiSpeed);
		GCCloseClientSend(aIndex,0);
		return;
	}
	...
	{
		gLog.Output(LOG_HACK,"[%s][%s] Client Speed Hack Detectado [%d][%d]",lpObj->Account,lpObj->Name,lpObj->MagicSpeed,lpMsg->MagicSpeed);
		GCCloseClientSend(aIndex,0);
		return;
	}
		...
		if (sObj->Connected == OBJECT_ONLINE)
		{
			if (account[0] == sObj->Account[0])
			{
				if (!strncmp(account, sObj->Account, 10))
				{
					...
					{
						s_FakeOnline.OnAttackAlreadyConnected(&gObj[i]); //ZEUS UP18 FAKE
...
//#else   //ZEUS UP24 LUA
	...
	if (lpObj->LoginMessageSend == 0)
	{
		...
		memcpy(lpObj->Password,password,sizeof(lpObj->Password));   //ZEUS UP24 LUA
...
void GCServerCommandSend(int aIndex,int type,int x,int y,int Viewport) //ZEUS UP24 LUA
{
	PMSG_SERVER_COMMAND_SEND pMsg;
