...
// NIVEL MÁXIMO - MODIFICADO PARA 97x
#ifdef SEASON_97X 
#define MAX_CHARACTER_LEVEL 350 
#define MAX_USER_LEVEL 350 
#else 
#define MAX_CHARACTER_LEVEL 1000 
#define MAX_USER_LEVEL 1000 
#endif
...
{
	MESSAGE_STATE_MACHINE() { this->Clear(); }
	void Clear() { this->MsgCode = -1; this->SendUser = -1; this->MsgTime = 0; this->SubCode = 0; }
	int MsgCode;
	int SendUser;
	int MsgTime;
	int SubCode;
...
{
	MESSAGE_STATE_MACHINE_COMMON() { this->CommonStruct = new MESSAGE_STATE_MACHINE; for (int n = 0;n < MAX_MONSTER_SEND_MSG;n++) { this->ObjectStruct[n] = this->CommonStruct; } }
	MESSAGE_STATE_MACHINE& operator[](int index) { return (*this->ObjectStruct[index]); }
...
{
	MESSAGE_STATE_MACHINE_HEADER() { this->CommonStruct = new MESSAGE_STATE_MACHINE_COMMON; for (int n = 0;n < MAX_OBJECT;n++) { this->ObjectStruct[n] = this->CommonStruct; } }
	MESSAGE_STATE_MACHINE_COMMON& operator[](int index) { return (*this->ObjectStruct[index]); }
...
{
	MESSAGE_STATE_ATTACK_MACHINE() { this->Clear(); }
	void Clear() { this->MsgCode = -1; this->SendUser = -1; this->MsgTime = 0; this->SubCode = 0; this->SubCode2 = 0; }
	int MsgCode;
	int SendUser;
	int MsgTime;
	int SubCode;
...
{
	MESSAGE_STATE_ATTACK_MACHINE_COMMON() { this->CommonStruct = new MESSAGE_STATE_ATTACK_MACHINE; for (int n = 0;n < MAX_MONSTER_SEND_ATTACK_MSG;n++) { this->ObjectStruct[n] = this->CommonStruct; } }
	MESSAGE_STATE_ATTACK_MACHINE& operator[](int index) { return (*this->ObjectStruct[index]); }
...
{
	MESSAGE_STATE_ATTACK_MACHINE_HEADER() { this->CommonStruct = new MESSAGE_STATE_ATTACK_MACHINE_COMMON; for (int n = 0;n < MAX_OBJECT;n++) { this->ObjectStruct[n] = this->CommonStruct; } }
	MESSAGE_STATE_ATTACK_MACHINE_COMMON& operator[](int index) { return (*this->ObjectStruct[index]); }
...
struct ACTION_STATE
{
	...
	void Reset() { this->Attack = false; this->Move = false; this->Emotion = EMOTION_REST; this->EmotionCount = 0; }
	void Update() { if (this->EmotionCount > 0) { this->EmotionCount--; } else { this->Emotion = EMOTION_REST; } }
};
...
// Forward Declarations (CRÍTICO - ANTES DE OBJECTSTRUCT)
...
// OBJECTSTRUCT - Variables SIEMPRE VISIBLES (usar #if en el código)
struct OBJECTSTRUCT
{
	int Index;
	int Connected;
...
{
	OBJECTSTRUCT_HEADER() { this->CommonStruct = new OBJECTSTRUCT; for (int n = 0;n < MAX_OBJECT;n++) { this->ObjectStruct[n] = this->CommonStruct; } }
	OBJECTSTRUCT& operator[](int index) { return (*this->ObjectStruct[index]); }
...
extern int gObjTotalGameMasters;