...
void Console(eLogColor color,char* text,...) // OK
{
	...
	WriteConsole(hConsole,log,strlen(log),0,0);
...
void PostMessage5(char* name,char* message,char* text) // OK
{
	#if(GAMESERVER_UPDATE>=701)
...
{
	std::random_device rd; // Creamos el dispositivo
	seed = std::mt19937(rd()); // Lo ejecutamos con rd()
