...
LRESULT CALLBACK WndProc(HWND hWnd,UINT message,WPARAM wParam,LPARAM lParam) // OK
{
	switch(message)
	{
		case WM_COMMAND:
			switch(LOWORD(wParam))
			{
				case IDM_ABOUT:
					...
					break;
				case IDM_EXIT:
					if(MessageBox(0,"Você tem certeza de terminar o JoinServer?", "ATENÇÃO QUER DESLIGAR",MB_YESNO | MB_ICONQUESTION) == IDYES)
					{
						DestroyWindow(hWnd);
					}
					break;
		...
		case WM_TIMER:
			...
			break;
		case WM_CLOSE:
			if (MessageBox(0, "Você tem certeza de terminar o JoinServer?", "ATENÇÃO QUER DESLIGAR", MB_OKCANCEL) == IDOK)
			{
				DestroyWindow(hWnd);
			}
			break;
		case WM_DESTROY:
			PostQuitMessage(0);
			break;
			case WM_PAINT:     //zeus up24 banner personalizado
		{
			...
			BitBlt(hdc,0,0,1500,80,hMemDC,0,0,SRCCOPY);
