...
void CLoadBitmapA(char * Folder, int Code, int Arg3, int Arg4, int Arg5, int Arg6)
{
	...
	LoadBitmapA("Logo\\New_Charactertk01.tga", 31, GL_NEAREST, GL_CLAMP, 1, 0);
}
...
void cCreateCharacter99b()
{
	if(*(DWORD*)(MAIN_SCREEN_STATE) == ObjState::SwitchCharacter)
	{
		if ( *(BYTE *)((int)sub_482B70() + 33948))
		{
			...
			if ( cTestPos >= 41 )
			{
				cTestPos = 41; //Posição do new character
			}
			RenderBitmap(31, 160.0, cTestPos - 30, 360.0, 330.0, 0, 0, 0.9375, 1.0, 1, 1, 0.0);

			if (SEASON3B_CheckMouseIn(372.0, cTestPos + 110, 23.0, 15.0))	
			{
				if (IsPress(1))	
				{
					...
					PlayBuffer(25, 0, 0);  
					RenderBitmap(26, 372.0, cTestPos + 110, 23.0, 15.0, 0, 0, 0.59375, 0.625, 1, 1, 0.0);//<<
				}
			}
			if (SEASON3B_CheckMouseIn(392.0, cTestPos + 110, 23.0, 15.0))	
			{
				if (IsPress(1))	
				{
					...
					PlayBuffer(25, 0, 0);
					RenderBitmap(27, 392.0, cTestPos + 110, 23.0, 15.0, 0, 0, 0.59375, 0.625, 1, 1, 0.0);//>> 
			...
			case 0:
				...
				DrawFormat(eWhite, 300, cTestPos + 115, 50, 3, pGetTextLine(pTextThis,(20))); 
			break;
			case 1:
				DrawFormat(eWhite, 300, cTestPos + 115, 50, 3, pGetTextLine(pTextThis,(21)));
			break;
			case 2:
				DrawFormat(eWhite, 300, cTestPos + 115, 50, 3, pGetTextLine(pTextThis,(22)));
			break;
			case 3:
				DrawFormat(eWhite, 300, cTestPos + 115, 50, 3, pGetTextLine(pTextThis,(23)));
			break;
			case 4:
			DrawFormat(eWhite, 300, cTestPos + 115, 50, 3, pGetTextLine(pTextThis,(24))); //Dark Lord
			break;
			case 5:
				DrawFormat(eWhite, 300, cTestPos + 115, 50, 3, pGetTextLine(pTextThis,(1687)));
			break;
			case 6:
				DrawFormat(eWhite, 300, cTestPos + 115, 50, 3, pGetTextLine(pTextThis,(3150)));
			break;
			}
			if(cPage == MAX_CREATE_CHARACTER)
			{
				EnableAlphaTest(true);
				glColor4f(0.0, 0.0, 0.0, 0.8);
				//pDrawBarForm(290, cTestPos + 102, 19.0, 20.0, 0.0f, 0);
				EnableAlphaBlend();
				EnableAlphaTest(1);
				glColor4f(1.0, 1.0, 1.0, 1.0);
			}
					
			CUIControl_SetPosition(dword_E8C544, 290, cTestPos + 102);//Textos


			if (SEASON3B_CheckMouseIn(228.0, cTestPos + 240, 80.0, 19.0))	
			{ 
				...
				RenderBitmap(28, 228.0, cTestPos + 240, 80.0, 19.0, 0, 0, 0.90625, 0.71875, 1, 1, 0.0);
			}	
			if (SEASON3B_CheckMouseIn(340.0, cTestPos + 240, 80.0, 19.0))	
			{
				if (GetKeyState(1) & 0x8000)
				{
					int v47 = (int)sub_482B70( ) + 33936;
				...
				RenderBitmap(29, 340.0, cTestPos + 240, 80.0, 19.0, 0, 0, 0.90625, 0.71875, 1, 1, 0.0);
...
void CRenderCreateCharacter(GLint a1, int a2, GLsizei width, GLsizei height)
{
	...
	if ( *(BYTE *)((int)sub_482B70() + 33948))
	{
		cTestPos = cTestPos + 10; // position of picture character // Posição do personagem select char
		if ( cTestPos >= 41 )
		{
			cTestPos = 41;
		}			
		BeginOpengl(287,cTestPos + 45,75,50);
	}
	else
	{
		cTestPos = -256.0;
...
void cSelectCharacter99b()
{
	if(*(DWORD*)(MAIN_SCREEN_STATE) == ObjState::SwitchCharacter)
	{
		if ( !*(BYTE *)((int)sub_482B70() + 33948))
		{
			...
			RenderBitmap(16, 234.0, CCCharacter - 55 , 172.0, 148.0, 0, 0, 0.671875, 0.578125, 1, 1, 0.0);	
			RenderBitmap(18, CDelete - 35, 132.0, 179.0, 61.0, 0, 0, 0.69921875, 0.953125, 1, 1, 0.0);  
			RenderBitmap(22, COK + 35, 132.0, 179.0, 61.0, 0, 0, 0.69921875, 0.953125, 1, 1, 0.0); 

			if (SEASON3B_CheckMouseIn(264.0, CCCharacter + 55, 70.0, 30.0))	
			{
				if (GetKeyState(1) & 0x8000)
				{		
					PlayBuffer(25, 0, 0);
				...
				RenderBitmap(17, 282.2, CCCharacter + 47.2, 75.0, 35.2, 0, 0, 0.875, 0.71875, 1, 1, 0.0); 
			}
		
			if (SEASON3B_CheckMouseIn(COK + 40.1, 153.0, 62.0, 15.0))	
			{
				...
				RenderBitmap(21, COK + 40.2, 153.0, 62.0, 15.0, 0, 0, 0.9375, 0.5625, 1, 1, 0.0);

			}	
			
			if (SEASON3B_CheckMouseIn(CDelete + 75, 153.0, 62.0, 15.0))	
			{
				SelectedCharacter = SelectedHero;
				if (GetKeyState(1) & 0x8000)
				{
				...
				RenderBitmap(19, CDelete + 75, 153.0, 62.0, 15.0, 0, 0, 0.9375, 0.5625, 1, 1, 0.0); 
			}		
		}		
		RenderBitmap(12, 448.0, 0.0, 192.0, 37.0, 0, 0, 0.75, 0.578125, 1, 1, 0.0);	
		RenderBitmap(12, 0.0, 0.0, 192.0, 37.0, 0.75, 0, -0.75, 0.578125, 1, 1, 0.0);
...
void CRenderCharacterCircle(int a1, int a2, int a3, int color, int a5, float a6, int a7)
{
	...
	RenderCircle(32142, a2, 50.0, 70.0, 200.0, v66, 0.0, 0.0); //Effect
}
...
{
	if ( *(int*)0x0E61E18 == 74) //Select Server
	{
...
void CSceneA::Load()
{
	...
	//SetDword(0x004D75F0, 94);//Select Server
	//SetCompleteHook(0xE8, 0x005EDDA9,&RenderSSCObjectVisual);//Select Server
	...
	//	SetDword(0x004D75F0, 94);//Select Server
		//SetCompleteHook(0xE8, 0x004D76FB, &LoginScreen);//Select Server
	   // SetCompleteHook(0xE8, 0x004D79C3, &LoginScreen);//Select Server
	//	SetDword((0x004D7A0E), 480);//Select Server
	//    SetByte((0x004D7A18), 0);//Select Server
		}
...
//Select Server/Character
	...
	SetOp(0x0057F159, (LPVOID)SelectCharacterSize1, ASM::JMP);
	SetOp(0x0057F148, (LPVOID)SelectCharacterSize2, ASM::JMP);
	SetOp(0x0057F16A, (LPVOID)SelectCharacterSize3, ASM::JMP);
	SetOp(0x0057F137, (LPVOID)SelectCharacterSize4, ASM::JMP);
	SetOp(0x0057F17B, (LPVOID)SelectCharacterSize5, ASM::JMP);
}