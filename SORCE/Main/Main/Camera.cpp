...
CCamera::CCamera() // OK
{
  ...
  this->m_Zoom.MinPercent = 30.0;

  this->m_Zoom.MaxPercent = 300.0;

  this->m_Zoom.Precision = 1.5;
...
void CCamera::Toggle() // OK
{
  if (*(DWORD *)(MAIN_SCREEN_STATE) == 5) {
    ...
    if (((this->m_Default.IsLoad == 0) ? (this->m_Default.IsLoad++)
                                       : this->m_Default.IsLoad) == 0) {
...
bool CCamera::getEnabled() { return this->m_Enable != 0; }
...
{
  if (this->m_Enable == 0 || this->m_IsMove != 0 ||
      *(DWORD *)(MAIN_SCREEN_STATE) != 5) {
    return;
  }

  this->m_Zoom.MinLimit =
      (this->m_Default.Zoom / 100) * this->m_Zoom.MinPercent;

  this->m_Zoom.MaxLimit =
      (this->m_Default.Zoom / 100) * this->m_Zoom.MaxPercent;

  if (((int)lpMouse->mouseData) > 0) {
    if ((*this->m_Address.Zoom) >= this->m_Zoom.MinLimit) {
      SetFloat((DWORD)this->m_Address.Zoom,
               ((*this->m_Address.Zoom) - this->m_Zoom.Precision));
    }
  }
...
{
  double posZFactor = abs(((*this->m_Address.PosZ) - 150.0f));

  SetDouble((DWORD)this->m_Address.ClipX[0], (8000.0f + (posZFactor * 6.0f)));

  SetDouble((DWORD)this->m_Address.ClipX[1], (8000.0f + (posZFactor * 6.0f)));

  SetFloat((DWORD)this->m_Address.ClipY[0],
           (10000.0f + ((float)posZFactor * 6.0f)));

  SetFloat((DWORD)this->m_Address.ClipY[1],
           (10000.0f + ((float)posZFactor * 6.0f)));

  SetFloat((DWORD)this->m_Address.ClipGL,
           (4000.0f + ((float)posZFactor * 6.0f)));
}
