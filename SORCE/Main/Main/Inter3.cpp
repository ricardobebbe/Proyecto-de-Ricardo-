...
void PetHPBarRemake(signed int x, signed int y, int nameIndex, signed int hp, signed int hpmax, char a9)
{
	...
	pDrawBarForm(v17 - 2, v16 - 2, 53.0, 7.0, 0.0, 0);
	pGLSwitchBlend();
	pGLSwitch();
	...
	green = (double)y;
	red = (double)x;
	pDrawGUI(31624, red, green, 57.0, 23.0);
	pGLSwitchBlend();
	pGLSwitch();
	glColor3f(1.0, 1.0, 1.0);
	pSetBlend(false);
	...
	pSetBlend(true);
	glColor4f(1.0, 1.0, 1.0, 1.0);
	v15 = (double)hp / (double)hpmax * 49.0;
	
	pDrawGUI(31625, v17, v16, v15, 3.0);
	pSetBackgroundTextColor(pTextThis(), 0, 0, 0, 255);
...
void PetSpiritHPBarRemake(signed int x, signed int y, int nameIndex, signed int hp, signed int hpmax, char a9)
{
	...
	x =  145.0f - reposicionX - gInterface.PetX -a3;  //Zeus UP21 PETHP CORVO
	y = 19;
	...
	pDrawText(pTextThis(), x + 28, y + 5,  (LPCTSTR)nameIndex, 0, 0, (LPINT)8, 0);  //Zeus UP21 PETHP CORVO
...
__declspec(naked) void DrawInterface97DisableR()
{
	static DWORD RButtonAddress = 0x00895C0C;
	_asm{jmp RButtonAddress}
}
...
bool MouseClick(int a1, int a2, int a3, int a4)
{
	...
	//	if ( gInterface.CheckWindow(77) )
		...
		//	if( gInterface.CheckWindow(ExpandInventory) )  //PASSAT ITENS PRO BAU
			{
		//		a3 = Return2X(TransFor2X(640) - (Width * 2));
			}
			//else
			...
			//return pCursorX > t && pCursorX < a3 && pCursorY < a4;  //PASSAT ITENS PRO BAU
		}
		...
		//else
		{
			//return pCursorX < a3 && pCursorY < a4;      //PASSAT ITENS PRO BAU
		}
	}
//	else
	{
	//	if ( gInterface.CheckWindow(13) && gInterface.CheckWindow(Character)    //JOGAR ITENS NO CHAO
		/*|| gInterface.CheckWindow(8)
		|| gInterface.CheckWindow(12)
		|| gInterface.CheckWindow(9)
		|| gInterface.CheckWindow(7)
		...
		//else
		{
		//	if ( gInterface.CheckWindow(16) && (gInterface.CheckWindow(4)||gInterface.CheckWindow(69)) )      //JOGAR ITENS NO CHAO
			{
			//	a3 = 260;
			}
			//else
			{
				//if ( gInterface.CheckWindow(16) && gInterface.CheckWindow(11) )     //JOGAR ITENS NO CHAO
				{
				//	a3 = 260;
				}
				//else
				{
				//	if ( gInterface.CheckWindow(27) )           //JOGAR ITENS NO CHAO
					{
					//	a3 = 260;
					}
				//	else
					{
						//if ( gInterface.CheckWindow(13)          //JOGAR ITENS NO CHAO
						/*|| gInterface.CheckWindow(16)
						|| gInterface.CheckWindow(3)
						|| gInterface.CheckWindow(21)
						|| gInterface.CheckWindow(6)
						...
						//else
						{
							//if ( gInterface.CheckWindow(79) )  //JOGAR ITENS NO CHAO
							{
							//	if ( gInterface.CheckWindow(80) )
								{
							//		a3 = 260;
...
__declspec(naked) void PartyWindowsMov()
{
...
_declspec(naked) void GuildWarPosition() //ZEUS UP18 POSIÇÃO GUILD WAR DOWGRAND 97D 
{
...
//--------------------------------
	...
	SetJmp((LPVOID)0x0084B95E, PartyWindowsMov);
	//-- Windows Command
	...
	SetCompleteHook(0xE9,0x00895B8D,&DrawInterface97DisableR);
	//--
...
//==================================
	...
	SetCompleteHook(0xE9, 0x00777008, &GuildWarPosition);//ZEUS UP18 POSIÇÃO GUILD WAR
	MemorySet(0x00776FE4,0x90,0x5); //ZEUS UP18 RETIRA IMAGEM GUILD WAR
}
