...
//--
void CProtect::CheckClientFile() //ZEUS UP20 MAXIMO DE ABRIR Main.exe
{
	...
	if (this->m_MainInfo.gMaxGameInstances == 0)
   {
	...
	if(_stricmp(ConvertModuleFileName(name),this->m_MainInfo.ClientName) != 0)
	{
	 ...
	 if (this->IsProcessRunning("main.exe", this->m_MainInfo.gMaxGameInstances))
   {
...
//--
bool CProtect::IsProcessRunning(const char *ProcessName, int maxProcess)  //ZEUS UP20 MAXIMO DE ABRIR Main.exe
{
