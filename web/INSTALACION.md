...
## Tienda web (VIP + Alas) con MercadoPago

1. **Base de datos**: ejecutá `sql/02_webshop.sql` sobre la base `MuOnline`.
   Crea las tablas `web_pedidos` (compras) y `web_noticias` (noticias de la web).

2. **Credenciales**: editá `Configuraciones/Shop.php` y completá:
   - `$MP_ACCESS_TOKEN` → Access Token de producción de MercadoPago.
   - `$SHOP_URL_BASE` → tu dominio, sin barra final (ej. `https://www.mumisiones.com`).

3. **Webhook**: en MercadoPago → *Tus integraciones → Webhooks*, cargá la URL
   `https://TU-DOMINIO/api/mp_webhook.php` y activá el evento **Pagos**.
   El webhook nunca confía en el aviso: vuelve a consultar el pago a la API y
   además compara el importe antes de entregar.

4. **Catálogo**: los productos se definen en el array `$SHOP_PRODUCTOS` de
   `Configuraciones/Shop.php`. Cada producto es:
   - `tipo => 'vip'` con `dias` (suma días de VIP en `MEMB_INFO`), o
   - `tipo => 'item'` con `hex` (los bytes del item, tal como los guarda tu
     serverfile). **Reemplazá los hex de ejemplo por los de tu server.**

5. **Tamaño del baúl**: si tu server usa otro formato, ajustá
   `$SHOP_BYTES_SLOT` (10 por defecto) y `$SHOP_BYTES_CAPACIDAD` (1080).

### Cómo funciona la entrega
- VIP: se activa al instante, sumando días sobre el vencimiento actual.
- Items: se insertan en el primer slot libre del baúl. Si el jugador está
  **online** o el baúl está **lleno**, el pedido queda en
  `retenido_falta_espacio` y el jugador puede reintentarlo solo desde
  **Mis pedidos** (`ir.php?amz=MisPedidos`).
- Un pedido ya entregado nunca se entrega dos veces (control por estado).

## Actualización final (resets, stats y seguridad)

- **Configuraciones/Servidor.php**: nuevo archivo con la IP y el puerto del GameServer.
  Está protegido por `Configuraciones/.htaccess` y ya no se muestra en la web pública.
  Se edita desde *Panel de Administración > Configuración del sitio > GameServer (privado)*.
- **Modulos/Reset.php** (`ir.php?amz=Reset`): reset web con chequeo de offline, level mínimo,
  costo en zen y reinicio de stats a la base de cada clase. Variables editables arriba del archivo
  (`$RS_PUNTOS`, `$RS_MAX`).
- **Modulos/Stats.php** (`ir.php?amz=Stats`): compra de puntos libres con zen
  (`$ST_PAQUETE`, `$ST_COSTO`, `$ST_MAXCOMP`).
- **Panel/Salir.php**: reescrito, ya no incluye `Conexion.php` (se evita el error de funciones duplicadas)
  y rompe el iframe al cerrar sesión.
